...
public class UnlockTracker {
   ...
   public static void retroactiveUnlock() {
      ...
      appendRetroactiveUnlockList(
         AbstractPlayer.PlayerClass.IRONCLAD,
         unlockProgress.getInteger(AbstractPlayer.PlayerClass.IRONCLAD.toString() + "UnlockLevel", -1),
         bundle,
         cardKeys,
         relicKeys
      );
      appendRetroactiveUnlockList(
         ...
         unlockProgress.getInteger(AbstractPlayer.PlayerClass.THE_SILENT.toString() + "UnlockLevel", -1),
         bundle,
         cardKeys,
         relicKeys
      );
      appendRetroactiveUnlockList(
         ...
         unlockProgress.getInteger(AbstractPlayer.PlayerClass.DEFECT.toString() + "UnlockLevel", -1),
         bundle,
         cardKeys,
         relicKeys
      );
      appendRetroactiveUnlockList(
         ...
         unlockProgress.getInteger(AbstractPlayer.PlayerClass.WATCHER.toString() + "UnlockLevel", -1),
         bundle,
         cardKeys,
         relicKeys
      );
      ...
      for (String kx : relicKeys) {
         if (unlockPref.getInteger(kx) != 2) {
            unlockPref.putInteger(kx, 2);
            changed = true;
            logger.info("RETROACTIVE RELIC UNLOCK: " + kx);
         }
      }

      if (isCharacterLocked("Watcher")
         && !isCharacterLocked("Defect")
         && (isAchievementUnlocked("RUBY") || isAchievementUnlocked("EMERALD") || isAchievementUnlocked("SAPPHIRE"))) {
   ...
   private static void appendRetroactiveUnlockList(
      AbstractPlayer.PlayerClass c, int lvl, ArrayList<AbstractUnlock> bundle, ArrayList<String> cardKeys, ArrayList<String> relicKeys
   ) {
      while (lvl > 0) {
         for (AbstractUnlock u : getUnlockBundle(c, lvl - 1)) {
            if (u.type == AbstractUnlock.UnlockType.RELIC) {
               logger.info(u.key + " should be unlocked.");
               relicKeys.add(u.key);
            } else if (u.type == AbstractUnlock.UnlockType.CARD) {
               logger.info(u.key + " should be unlocked.");
               cardKeys.add(u.key);
            }
         }
   ...
   public static void countUnlockedCards() {
      ...
      for (String sxxxx : tmp) {
         if (!isRelicLocked(sxxxx)) {
            count++;
         }
      }
   ...
   public static boolean isCharacterLocked(String key) {
      if (key.equals("The Silent") && Settings.isDemo) {
         ...
         return Settings.isAlpha ? false : lockedCharacters.contains(key);
      }
   }
   ...
   private static float getAchievementProgress() {
      ...
      float retVal = (float)sum / StatsScreen.achievements.items.size();
   ...
   private static float getSeenCardsProgress() {
      ...
      for (Entry<String, AbstractCard> c : CardLibrary.cards.entrySet()) {
         if (c.getValue().isSeen) {
            sum++;
         }
      }

      float retVal = (float)sum / CardLibrary.cards.size();
   ...
   private static float getSeenRelicsProgress() {
      float retVal = (float)RelicLibrary.seenRelics / RelicLibrary.totalRelicCount;
