...
public class SingleCardViewPopup {
   ...
   public void render(SpriteBatch sb) {
      ...
      if (this.allowUpgradePreview()) {
         ...
         if (Settings.isControllerMode) {
            sb.draw(
               ...
               this.upgradeHb.cX - 132.0F * Settings.scale - 32.0F,
               -32.0F + 67.0F * Settings.scale,
               32.0F,
               32.0F,
               64.0F,
               64.0F,
      ...
      if (this.betaArtHb != null && Settings.isControllerMode) {
         sb.draw(
            ...
            this.betaArtHb.cX - 132.0F * Settings.scale - 32.0F,
            -32.0F + 67.0F * Settings.scale,
            32.0F,
            32.0F,
            64.0F,
   ...
   public void renderCardBack(SpriteBatch sb) {
      ...
      if (tmpImg != null) {
         this.renderHelper(sb, Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, tmpImg);
      } else {
   ...
   private void renderPortrait(SpriteBatch sb) {
      ...
      if (this.card.isLocked) {
         ...
         this.renderHelper(sb, Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F + 136.0F * Settings.scale, img);
      } else if (this.portraitImg != null) {
         sb.draw(
            this.portraitImg,
            Settings.WIDTH / 2.0F - 250.0F,
            Settings.HEIGHT / 2.0F - 190.0F + 136.0F * Settings.scale,
            250.0F,
            190.0F,
            500.0F,
            380.0F,
         ...
         sb.draw(
            this.card.jokePortrait,
            Settings.WIDTH / 2.0F - this.card.portrait.packedWidth / 2.0F,
            Settings.HEIGHT / 2.0F - this.card.portrait.packedHeight / 2.0F + 140.0F * Settings.scale,
            this.card.portrait.packedWidth / 2.0F,
            this.card.portrait.packedHeight / 2.0F,
            this.card.portrait.packedWidth,
            this.card.portrait.packedHeight,
   ...
   private void renderFrame(SpriteBatch sb) {
      ...
      if (tmpImg != null) {
         ...
         if (img != null) {
            ...
            this.renderHelper(sb, Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, ImageMaster.CARD_FRAME_SKILL_COMMON_L);
         }
      }

      this.renderDynamicFrame(sb, Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, tOffset, tWidth);
   }
   ...
   private void renderDescriptionCN(SpriteBatch sb) {
      if (!this.card.isLocked && this.card.isSeen) {
         ...
         draw_y += this.card.description.size() * font.getCapHeight() * 0.775F - font.getCapHeight() * 0.375F;
            ...
            for (String tmp : this.card.description.get(i).getCachedTokenizedTextCN()) {
               ...
               if (!tmp.isEmpty() && tmp.charAt(0) == '*') {
                  ...
                  FontHelper.renderRotatedText(
                     ...
                     i * 1.53F * -font.getCapHeight() + draw_y - this.current_y + -12.0F,
                     0.0F,
                     true,
                     Settings.GOLD_COLOR
                  );
         ...
         FontHelper.renderFontCentered(
            sb, FontHelper.largeCardFont, "? ? ?", Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F - 195.0F * Settings.scale, Settings.CREAM_COLOR
         );
      }
   }
   ...
   private void renderSmallEnergy(SpriteBatch sb, TextureAtlas.AtlasRegion region, float x, float y) {
      ...
      sb.draw(
         region.getTexture(),
         this.current_x + x * Settings.scale * this.drawScale + region.offsetX * Settings.scale - 4.0F * Settings.scale,
         this.current_y + y * Settings.scale * this.drawScale + 280.0F * Settings.scale,
         0.0F,
         0.0F,
         region.packedWidth,
         region.packedHeight,
   ...
   private void renderCardTypeText(SpriteBatch sb) {
      ...
      FontHelper.renderFontCentered(
         sb, FontHelper.panelNameFont, label, Settings.WIDTH / 2.0F + 3.0F * Settings.scale, Settings.HEIGHT / 2.0F - 40.0F * Settings.scale, CARD_TYPE_COLOR
      );
   }
   ...
   private void renderTitle(SpriteBatch sb) {
      if (this.card.isLocked) {
         FontHelper.renderFontCentered(
            sb, FontHelper.SCP_cardTitleFont_small, TEXT[4], Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F + 338.0F * Settings.scale, Settings.CREAM_COLOR
         );
         ...
         if (isViewingUpgrade && !this.allowUpgradePreview()) {
            FontHelper.renderFontCentered(
               ...
               this.card.name,
               Settings.WIDTH / 2.0F,
               Settings.HEIGHT / 2.0F + 338.0F * Settings.scale,
         ...
         FontHelper.renderFontCentered(
            sb, FontHelper.SCP_cardTitleFont_small, TEXT[5], Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F + 338.0F * Settings.scale, Settings.CREAM_COLOR
         );
      }
   }
   ...
   private void renderCost(SpriteBatch sb) {
      if (!this.card.isLocked && this.card.isSeen) {
         if (this.card.cost > -2) {
            ...
            if (tmpImg != null) {
               this.renderHelper(sb, Settings.WIDTH / 2.0F - 270.0F * Settings.scale, Settings.HEIGHT / 2.0F + 380.0F * Settings.scale, tmpImg);
            }
         }
         ...
         switch (this.card.cost) {
            ...
            case -1:
               FontHelper.renderFont(
                  sb, FontHelper.SCP_cardEnergyFont, "X", Settings.WIDTH / 2.0F - 292.0F * Settings.scale, Settings.HEIGHT / 2.0F + 404.0F * Settings.scale, c
               );
               break;
            case 0:
            default:
               FontHelper.renderFont(
                  sb,
                  FontHelper.SCP_cardEnergyFont,
                  Integer.toString(this.card.cost),
                  Settings.WIDTH / 2.0F - 292.0F * Settings.scale,
                  Settings.HEIGHT / 2.0F + 404.0F * Settings.scale,
                  c
               );
               break;
            case 1:
   ...
   private void renderHelper(SpriteBatch sb, float x, float y, TextureAtlas.AtlasRegion img) {
      if (img != null) {
         sb.draw(
            img,
            x + img.offsetX - img.originalWidth / 2.0F,
   ...
   private void renderBetaArtToggle(SpriteBatch sb) {
      if (this.betaArtHb != null) {
         ...
         sb.draw(
            ...
            this.betaArtHb.cX - 80.0F * Settings.scale - 32.0F,
            this.betaArtHb.cY - 32.0F,
            32.0F,
            32.0F,
            64.0F,
            64.0F,
         ...
         if (this.betaArtHb.hovered) {
            FontHelper.renderFont(
               ...
               TEXT[14],
               this.betaArtHb.cX - 45.0F * Settings.scale,
               this.betaArtHb.cY + 10.0F * Settings.scale,
   ...
   private void renderUpgradeViewToggle(SpriteBatch sb) {
      ...
      sb.draw(
         ImageMaster.CHECKBOX,
         this.upgradeHb.cX - 80.0F * Settings.scale - 32.0F,
         this.upgradeHb.cY - 32.0F,
         32.0F,
         32.0F,
         64.0F,
         64.0F,
      ...
      if (this.upgradeHb.hovered) {
         FontHelper.renderFont(
            ...
            FontHelper.cardTitleFont,
            TEXT[6],
            this.upgradeHb.cX - 45.0F * Settings.scale,
            this.upgradeHb.cY + 10.0F * Settings.scale,
            Settings.BLUE_TEXT_COLOR
         );
      } else {
         FontHelper.renderFont(
            sb, FontHelper.cardTitleFont, TEXT[6], this.upgradeHb.cX - 45.0F * Settings.scale, this.upgradeHb.cY + 10.0F * Settings.scale, Settings.GOLD_COLOR
         );
      }
