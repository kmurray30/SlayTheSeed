...
public class Matrix3 implements Serializable {
   ...
   public Matrix3 setToRotation(float degrees) {
      return this.setToRotationRad((float) (Math.PI / 180.0) * degrees);
   }
   ...
   public Matrix3 rotate(float degrees) {
      return this.rotateRad((float) (Math.PI / 180.0) * degrees);
   }
   ...
   public float getRotation() {
      return (180.0F / (float)Math.PI) * (float)Math.atan2(this.val[1], this.val[0]);
   }
