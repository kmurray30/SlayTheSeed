...
final class LinuxMouse {
   ...
   private void handleButtonPress(boolean grab, byte button, long nanos) {
      ...
      switch (button) {
         case 4:
            int var7 = 120;
            this.putMouseEvent(grab, (byte)-1, (byte)0, var7, nanos);
            this.accum_dz += var7;
            break;
         case 5:
            int var6 = -120;
            this.putMouseEvent(grab, (byte)-1, (byte)0, var6, nanos);
            this.accum_dz += var6;
            break;
         default:
   ...
   public boolean filterEvent(boolean grab, boolean warp_pointer, LinuxEvent event) {
      switch (event.getType()) {
         ...
         case 5:
            ...
            return true;
         case 6:
            this.handlePointerMotion(
               grab,
               warp_pointer,
               event.getButtonTime(),
               event.getButtonRoot(),
               event.getButtonXRoot(),
               event.getButtonYRoot(),
               event.getButtonX(),
               event.getButtonY()
            );
            return true;
