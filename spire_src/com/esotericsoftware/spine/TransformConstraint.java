   ...
   public void update() {
      ...
      for (int n = bones.size; i < n; i++) {
         ...
         if (rotateMix > 0.0F) {
            ...
            float c = bone.c;
            float d = bone.d;
            float r = MathUtils.atan2(tc, ta) - MathUtils.atan2(c, a) + this.data.offsetRotation * (float) (Math.PI / 180.0);
            if (r > (float) Math.PI) {
               r -= (float) (Math.PI * 2);
         ...
         if (translateMix > 0.0F) {
            ...
            bone.worldX = bone.worldX + (temp.x - bone.worldX) * translateMix;
            bone.worldY = bone.worldY + (temp.y - bone.worldY) * translateMix;
         }
         ...
         if (shearMix > 0.0F) {
            ...
            r = by + (r + this.data.offsetShearY * (float) (Math.PI / 180.0)) * shearMix;
