...
public class OrthoCachedTiledMapRenderer implements TiledMapRenderer, Disposable {
   ...
   public void setView(OrthographicCamera camera) {
      ...
      if (this.canCacheMoreW && this.viewBounds.x < this.cacheBounds.x - 1.0E-5F
         || this.canCacheMoreS && this.viewBounds.y < this.cacheBounds.y - 1.0E-5F
         || this.canCacheMoreE && this.viewBounds.x + this.viewBounds.width > this.cacheBounds.x + this.cacheBounds.width + 1.0E-5F
         || this.canCacheMoreN && this.viewBounds.y + this.viewBounds.height > this.cacheBounds.y + this.cacheBounds.height + 1.0E-5F) {
         this.cached = false;
      }
   }
   ...
   public void render() {
      ...
      for (int j = mapLayers.getCount(); i < j; i++) {
         ...
         for (int col = col1; col < col2; col++) {
            ...
            if (cell != null) {
               ...
               if (tile != null) {
                  ...
                  float x1 = col * layerTileWidth + tile.getOffsetX() * this.unitScale;
                  float y1 = row * layerTileHeight + tile.getOffsetY() * this.unitScale;
