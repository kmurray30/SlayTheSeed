...
public class POP3Client extends POP3 {
   ...
   public boolean login(String username, String password) throws IOException {
      if (this.getState() != 0) {
         return false;
      } else if (this.sendCommand(0, username) != 0) {
         return false;
      } else if (this.sendCommand(1, password) != 0) {
         return false;
      } else {
         this.setState(1);
         return true;
   ...
   public boolean noop() throws IOException {
      return this.getState() == 1 ? this.sendCommand(7) == 0 : false;
   }

   public boolean deleteMessage(int messageId) throws IOException {
      return this.getState() == 1 ? this.sendCommand(6, Integer.toString(messageId)) == 0 : false;
   }

   public boolean reset() throws IOException {
      return this.getState() == 1 ? this.sendCommand(8) == 0 : false;
   }
   ...
   public POP3MessageInfo listMessage(int messageId) throws IOException {
      if (this.getState() != 1) {
         ...
         return this.sendCommand(4, Integer.toString(messageId)) != 0 ? null : __parseStatus(this._lastReplyLine.substring(3));
      }
   }
   ...
   public POP3MessageInfo listUniqueIdentifier(int messageId) throws IOException {
      if (this.getState() != 1) {
         ...
         return this.sendCommand(11, Integer.toString(messageId)) != 0 ? null : __parseUID(this._lastReplyLine.substring(3));
      }
   }
   ...
   public Reader retrieveMessage(int messageId) throws IOException {
      if (this.getState() != 1) {
         ...
         return this.sendCommand(5, Integer.toString(messageId)) != 0 ? null : new DotTerminatedMessageReader(this._reader);
      }
   }
   ...
   public Reader retrieveMessageTop(int messageId, int numLines) throws IOException {
      if (numLines < 0 || this.getState() != 1) {
         ...
         return this.sendCommand(10, Integer.toString(messageId) + " " + Integer.toString(numLines)) != 0 ? null : new DotTerminatedMessageReader(this._reader);
      }
   }
}
