...
public class DoubleMetaphone implements StringEncoder {
   ...
   private int handleG(String value, DoubleMetaphone.DoubleMetaphoneResult result, int index, boolean slavoGermanic) {
      if (this.charAt(value, index + 1) == 'H') {
         ...
         if ((contains(value, index + 1, 2, "ER") || this.charAt(value, index + 1) == 'Y')
   ...
   private int handleJ(String value, DoubleMetaphone.DoubleMetaphoneResult result, int index, boolean slavoGermanic) {
      if (!contains(value, index, 4, "JOSE") && !contains(value, 0, 4, "SAN ")) {
         if (index == 0 && !contains(value, index, 4, "JOSE")) {
            result.append('J', 'A');
         } else if (!this.isVowel(this.charAt(value, index - 1))
            || slavoGermanic
   ...
   private int handleW(String value, DoubleMetaphone.DoubleMetaphoneResult result, int index) {
      if (contains(value, index, 2, "WR")) {
         ...
         if ((index != value.length() - 1 || !this.isVowel(this.charAt(value, index - 1)))
   ...
   private boolean conditionCH0(String value, int index) {
      if (index != 0) {
         ...
         return !contains(value, index + 1, 5, "HARAC", "HARIS") && !contains(value, index + 1, 3, "HOR", "HYM", "HIA", "HEM")
            ? false
   ...
   private boolean conditionL0(String value, int index) {
      return index == value.length() - 3 && contains(value, index - 1, 4, "ILLO", "ILLA", "ALLE")
         ? true
         : (contains(value, value.length() - 2, 2, "AS", "OS") || contains(value, value.length() - 1, 1, "A", "O")) && contains(value, index - 1, 4, "ALLE");
   }

   private boolean conditionM0(String value, int index) {
      return this.charAt(value, index + 1) == 'M'
         ? true
         : contains(value, index - 1, 3, "UMB") && (index + 1 == value.length() - 1 || contains(value, index + 2, 2, "ER"));
   }
