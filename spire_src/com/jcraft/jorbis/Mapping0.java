...
class Mapping0 extends FuncMapping {
   ...
   Object unpack(Info vi, Buffer opb) {
      ...
      if (opb.read(2) > 0) {
         ...
         if (info.submaps > 1) {
            for (int ix = 0; ix < vi.channels; ix++) {
               info.chmuxlist[ix] = opb.read(4);
               if (info.chmuxlist[ix] >= info.submaps) {
                  info.free();
                  return null;
               }
            }
         ...
         for (int ixx = 0; ixx < info.submaps; ixx++) {
            info.timesubmap[ixx] = opb.read(8);
            if (info.timesubmap[ixx] >= vi.times) {
               info.free();
               return null;
            }

            info.floorsubmap[ixx] = opb.read(8);
            if (info.floorsubmap[ixx] >= vi.floors) {
               info.free();
               return null;
            }

            info.residuesubmap[ixx] = opb.read(8);
            if (info.residuesubmap[ixx] >= vi.residues) {
               info.free();
               return null;
            }
         }
   ...
   synchronized int inverse(Block vb, Object l) {
      ...
      for (int ix = 0; ix < info.submaps; ix++) {
         int ch_in_bundle = 0;

         for (int j = 0; j < vi.channels; j++) {
            if (info.chmuxlist[j] == ix) {
               if (this.nonzero[j] != 0) {
         ...
         look.residue_func[ix].inverse(vb, look.residue_look[ix], this.pcmbundle, this.zerobundle, ch_in_bundle);
      }

      for (int ix = info.coupling_steps - 1; ix >= 0; ix--) {
         float[] pcmM = vb.pcm[info.coupling_mag[ix]];
         float[] pcmA = vb.pcm[info.coupling_ang[ix]];

         for (int jx = 0; jx < n / 2; jx++) {
            float mag = pcmM[jx];
            float ang = pcmA[jx];
            if (mag > 0.0F) {
               if (ang > 0.0F) {
      ...
      for (int ix = 0; ix < vi.channels; ix++) {
         float[] pcm = vb.pcm[ix];
         int submap = info.chmuxlist[ix];
         look.floor_func[submap].inverse2(vb, look.floor_look[submap], this.floormemo[ix], pcm);
      }
