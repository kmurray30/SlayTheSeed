...
public class ModelInfluencerPanel extends InfluencerPanel<ModelInfluencer> implements TemplatePickerPanel.Listener<Model>, EventManager.Listener {
   ...
   public void handle(int aEventType, Object aEventData) {
      if (aEventType == 0) {
         ...
         if (data[0] instanceof Model && this.value.models.removeValue((Model)data[0], true)) {
            this.value.models.add((Model)data[1]);
            this.pickerPanel.reloadTemplates();
            this.pickerPanel.setValue(this.value.models);
            this.editor.restart();
         }
      }
   }
