...
public abstract class CommandExampleBase implements TwirkListener {
   ...
   private CommandExampleBase.CommandType type;
   private USER_TYPE minPrivilidge;

   protected CommandExampleBase(CommandExampleBase.CommandType type) {
      this.type = type;
   ...
   public void onPrivMsg(TwitchUser sender, TwitchMessage message) {
      ...
      if (sender.getUserType().value >= this.minPrivilidge.value) {
         if (this.type == CommandExampleBase.CommandType.PREFIX_COMMAND) {
            ...
            for (String patternx : this.commandPattern) {
               if (content.contains(patternx)) {
                  this.performCommand(patternx, sender, message);
                  break;
               }
            }
         }
