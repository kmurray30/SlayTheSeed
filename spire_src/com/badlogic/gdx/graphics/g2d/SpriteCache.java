...
public class SpriteCache implements Disposable {
   ...
   public void beginCache(int cacheID) {
      if (this.currentCache != null) {
         ...
         SpriteCache.Cache oldCache = this.caches.removeIndex(cacheID);
   ...
   public int endCache() {
      if (this.currentCache == null) {
         ...
         if (cache.textures == null) {
            ...
            for (int n = this.counts.size; i < n; i++) {
            ...
            for (int n = cache.textureCount; i < n; i++) {
            ...
            SpriteCache.Cache lastCache = this.caches.get(this.caches.size - 1);
   ...
   public void draw(int cacheID) {
      if (!this.drawing) {
         ...
         SpriteCache.Cache cache = this.caches.get(cacheID);
         int verticesPerImage = this.mesh.getNumIndices() > 0 ? 4 : 6;
