...
public abstract class ColorInfluencer extends Influencer {
   ...
   public static class Random extends ColorInfluencer {
      ...
      public void activateParticles(int startIndex, int count) {
         ...
         for (int c = i + count * this.colorChannel.strideSize; i < c; i += this.colorChannel.strideSize) {
      ...
      public ColorInfluencer.Random copy() {
         return new ColorInfluencer.Random();
      }
   }
   ...
   public static class Single extends ColorInfluencer {
      ...
      public void activateParticles(int startIndex, int count) {
         ...
         for (int c = i + count * this.colorChannel.strideSize; i < c; l += this.lifeChannel.strideSize) {
      ...
      public void update() {
         ...
         for (int c = i + this.controller.particles.size * this.colorChannel.strideSize; i < c; l += this.lifeChannel.strideSize) {
            float lifePercent = this.lifeChannel.data[l];
            this.colorValue.getColor(lifePercent, this.colorChannel.data, i);
            this.colorChannel.data[i + 3] = this.alphaInterpolationChannel.data[a + 0]
               + this.alphaInterpolationChannel.data[a + 1] * this.alphaValue.getScale(lifePercent);
            i += this.colorChannel.strideSize;
            a += this.alphaInterpolationChannel.strideSize;
         }
      }
