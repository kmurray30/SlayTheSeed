...
public class DefaultFTPFileEntryParserFactory implements FTPFileEntryParserFactory {
         ...
         try {
            ...
            try {
               ...
               throw new ParserInitializationException(
                  parserClass.getName() + " does not implement the interface " + "org.apache.commons.net.ftp.FTPFileEntryParser.", var6
               );
   ...
   private FTPFileEntryParser createNTFTPEntryParser(FTPClientConfig config) {
      return (FTPFileEntryParser)(config != null && "WINDOWS".equals(config.getServerSystemKey())
         ? new NTFTPEntryParser(config)
         : new CompositeFileEntryParser(
            new FTPFileEntryParser[]{
               new NTFTPEntryParser(config), new UnixFTPEntryParser(config, config != null && "UNIX_LTRIM".equals(config.getServerSystemKey()))
            }
         ));
   }
   ...
   private FTPFileEntryParser createOS400FTPEntryParser(FTPClientConfig config) {
      return (FTPFileEntryParser)(config != null && "OS/400".equals(config.getServerSystemKey())
         ? new OS400FTPEntryParser(config)
         : new CompositeFileEntryParser(
            new FTPFileEntryParser[]{
               new OS400FTPEntryParser(config), new UnixFTPEntryParser(config, config != null && "UNIX_LTRIM".equals(config.getServerSystemKey()))
            }
         ));
   }
