...
public class Quaternion implements Serializable {
   ...
   public Quaternion setEulerAngles(float yaw, float pitch, float roll) {
      return this.setEulerAnglesRad(yaw * (float) (Math.PI / 180.0), pitch * (float) (Math.PI / 180.0), roll * (float) (Math.PI / 180.0));
   }
   ...
   public float getRollRad() {
      ...
      return pole == 0
         ? MathUtils.atan2(2.0F * (this.w * this.z + this.y * this.x), 1.0F - 2.0F * (this.x * this.x + this.z * this.z))
         : pole * 2.0F * MathUtils.atan2(this.y, this.w);
   }

   public float getRoll() {
      return this.getRollRad() * (180.0F / (float)Math.PI);
   }

   public float getPitchRad() {
      int pole = this.getGimbalPole();
      return pole == 0 ? (float)Math.asin(MathUtils.clamp(2.0F * (this.w * this.x - this.z * this.y), -1.0F, 1.0F)) : pole * (float) Math.PI * 0.5F;
   }

   public float getPitch() {
      return this.getPitchRad() * (180.0F / (float)Math.PI);
   }
   ...
   public float getYaw() {
      return this.getYawRad() * (180.0F / (float)Math.PI);
   }
   ...
   public boolean isIdentity(float tolerance) {
      return MathUtils.isZero(this.x, tolerance)
         && MathUtils.isZero(this.y, tolerance)
         && MathUtils.isZero(this.z, tolerance)
         && MathUtils.isEqual(this.w, 1.0F, tolerance);
   }
   ...
   public Quaternion setFromAxis(float x, float y, float z, float degrees) {
      return this.setFromAxisRad(x, y, z, degrees * (float) (Math.PI / 180.0));
   }
   ...
   public Quaternion setFromMatrix(boolean normalizeAxes, Matrix4 matrix) {
      return this.setFromAxes(
         normalizeAxes, matrix.val[0], matrix.val[4], matrix.val[8], matrix.val[1], matrix.val[5], matrix.val[9], matrix.val[2], matrix.val[6], matrix.val[10]
      );
   }
   ...
   public Quaternion setFromMatrix(boolean normalizeAxes, Matrix3 matrix) {
      return this.setFromAxes(
         normalizeAxes, matrix.val[0], matrix.val[3], matrix.val[6], matrix.val[1], matrix.val[4], matrix.val[7], matrix.val[2], matrix.val[5], matrix.val[8]
      );
   }
   ...
   public int hashCode() {
      ...
      return 31 * result + NumberUtils.floatToRawIntBits(this.z);
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return NumberUtils.floatToRawIntBits(this.w) == NumberUtils.floatToRawIntBits(other.w)
            && NumberUtils.floatToRawIntBits(this.x) == NumberUtils.floatToRawIntBits(other.x)
            && NumberUtils.floatToRawIntBits(this.y) == NumberUtils.floatToRawIntBits(other.y)
            && NumberUtils.floatToRawIntBits(this.z) == NumberUtils.floatToRawIntBits(other.z);
      }
   }
   ...
   public float getAxisAngle(Vector3 axis) {
      return this.getAxisAngleRad(axis) * (180.0F / (float)Math.PI);
   }
   ...
   public float getAngle() {
      return this.getAngleRad() * (180.0F / (float)Math.PI);
   }
   ...
   public float getAngleAround(float axisX, float axisY, float axisZ) {
      return this.getAngleAroundRad(axisX, axisY, axisZ) * (180.0F / (float)Math.PI);
   }
