...
public class Soul {
   ...
   public void update() {
      if (this.isCarryingCard()) {
         ...
         if (this.spawnStutterTimer > 0.0F) {
            this.spawnStutterTimer = this.spawnStutterTimer - Gdx.graphics.getDeltaTime();
            return;
         }
   ...
   private void updateMovement() {
      ...
      this.rotationRate = this.rotationRate + Gdx.graphics.getDeltaTime() * 800.0F;
      ...
      this.tmp.x = this.tmp.x * (Gdx.graphics.getDeltaTime() * this.currentSpeed);
      this.tmp.y = this.tmp.y * (Gdx.graphics.getDeltaTime() * this.currentSpeed);
      ...
      if (!this.isDone && this.vfxTimer < 0.0F) {
         ...
         if (this.controlPoints.size() > 3) {
            ...
            for (int i = 0; i < 20; i++) {
               ...
               Vector2 derp = this.crs.valueAt(this.points[i], i / 19.0F);
   ...
   private void updateBackUpTimer() {
      this.backUpTimer = this.backUpTimer - Gdx.graphics.getDeltaTime();
