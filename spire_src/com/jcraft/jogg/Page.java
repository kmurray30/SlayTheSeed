...
public class Page {
   ...
   public long granulepos() {
      ...
      foo = foo << 8 | this.header_base[this.header + 12] & 255;
      foo = foo << 8 | this.header_base[this.header + 11] & 255;
      foo = foo << 8 | this.header_base[this.header + 10] & 255;
      foo = foo << 8 | this.header_base[this.header + 9] & 255;
      foo = foo << 8 | this.header_base[this.header + 8] & 255;
      foo = foo << 8 | this.header_base[this.header + 7] & 255;
      return foo << 8 | this.header_base[this.header + 6] & 255;
   }
   ...
   void checksum() {
      ...
      for (int i = 0; i < this.header_len; i++) {
         crc_reg = crc_reg << 8 ^ crc_lookup[crc_reg >>> 24 & 0xFF ^ this.header_base[this.header + i] & 255];
      }

      for (int i = 0; i < this.body_len; i++) {
         crc_reg = crc_reg << 8 ^ crc_lookup[crc_reg >>> 24 & 0xFF ^ this.body_base[this.body + i] & 255];
      }
