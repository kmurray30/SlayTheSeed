   ...
   public void takeTurn() {
      ...
      switch (this.nextMove) {
         case 2:
            for (int i = 0; i < 3; i++) {
               AbstractDungeon.actionManager
                  .addToBottom(
                     new VFXAction(this, new ShockWaveEffect(this.hb.cX, this.hb.cY, Settings.BLUE_TEXT_COLOR, ShockWaveEffect.ShockWaveType.CHAOTIC), 0.75F)
                  );
         ...
         case 3:
            ...
            AbstractDungeon.actionManager
               .addToBottom(new ApplyPowerAction(AbstractDungeon.player, this, new VulnerablePower(AbstractDungeon.player, 1, true), 1));
   ...
   protected void getMove(int num) {
      if (this.currentHealth < this.maxHealth / 2 && !this.usedHaste) {
         ...
         if (!this.lastTwoMoves((byte)2)) {
            this.setMove((byte)2, AbstractMonster.Intent.ATTACK, this.damage.get(0).base, 3, true);
         } else {
            this.getMove(AbstractDungeon.aiRng.random(50, 99));
         }
      } else if (num < 80) {
         if (!this.lastMove((byte)4)) {
            this.setMove((byte)4, AbstractMonster.Intent.ATTACK_DEBUFF, this.damage.get(1).base);
