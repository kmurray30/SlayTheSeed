...
public class MappedObjectTransformer {
   ...
   static final Map<String, MappedObjectTransformer.MappedSubtypeInfo> className_to_subtype = new HashMap<>();
   ...
   static final Map<Integer, String> OPCODE_TO_NAME = new HashMap<>();
   static final Map<Integer, String> INSNTYPE_TO_NAME = new HashMap<>();
   ...
   public static void register(Class<? extends MappedObject> type) {
      if (!MappedObjectClassLoader.FORKED) {
         ...
         if (mapped != null && mapped.padding() < 0) {
            ...
            sizeof += padding;
            MappedObjectTransformer.MappedSubtypeInfo mappedType = new MappedObjectTransformer.MappedSubtypeInfo(
               className, fields, (int)sizeof, align, padding, cacheLinePadded
            );
         ...
         if (meta == null && !autoGenerateOffsets) {
            ...
            if (pointer != null && field.getType() != long.class) {
               throw new ClassFormatError(
                  "The @Pointer annotation can only be used on long fields. @Pointer field found: "
                     + className
                     + "."
                     + field.getName()
                     + ": "
                  ...
                  if (field.getType() == double.class) {
                     ...
                     if (byteLength < 0L) {
                        throw new IllegalStateException(
                           "invalid byte length for mapped ByteBuffer field: " + className + "." + field.getName() + " [length=" + byteLength + "]"
                        );
                     }
                  }
               ...
               if (field.getType() != ByteBuffer.class && advancingOffset % byteLength != 0L) {
                  ...
                  if (meta != null && meta.byteOffset() != -1L) {
                     if (meta.byteOffset() < 0L) {
                        throw new ClassFormatError(
                           "Invalid field byte offset: " + className + "." + field.getName() + " [byteOffset=" + meta.byteOffset() + "]"
                        );
                     }

                     if (pad != null) {
                        throw new ClassFormatError(
                           "A field byte offset cannot be specified together with cache-line padding: " + className + "." + field.getName()
                        );
                     }
                  ...
                  if (pad != null) {
                     ...
                     if (pad.after() && (byteOffset + byteLength) % CacheUtil.getCacheLineSize() != 0L) {
                        byteLengthPadded = byteLength + (CacheUtil.getCacheLineSize() - (byteOffset + byteLength) % CacheUtil.getCacheLineSize());
                     }
                  ...
                  if (PRINT_ACTIVITY) {
                     LWJGLUtil.log(
                        MappedObjectTransformer.class.getSimpleName()
                           + ": "
                           + className
                           + "."
                           + field.getName()
                  ...
                  return new MappedObjectTransformer.FieldInfo(
                     byteOffset, byteLength, byteLengthPadded, Type.getType(field.getType()), Modifier.isVolatile(field.getModifiers()), pointer != null
                  );
               }
            } else {
               throw new ClassFormatError(
                  "The volatile keyword is not supported for @Pointer or ByteBuffer fields. Volatile field found: "
                     + className
                     + "."
                     + field.getName()
                     + ": "
   ...
   static byte[] transformMappedAPI(String className, byte[] bytecode) {
      ClassWriter cw = new ClassWriter(2) {
         protected String getCommonSuperClass(String a, String b) {
            return (!MappedObjectTransformer.is_currently_computing_frames || a.startsWith("java/")) && b.startsWith("java/")
   ...
   private static ClassAdapter getMethodGenAdapter(final String className, ClassVisitor cv) {
      return new ClassAdapter(cv) {
         ...
         private void generateCapacity() {
            ...
            mv.visitVarInsn(25, 0);
            mv.visitMethodInsn(
               182, MappedObjectTransformer.MAPPED_OBJECT_JVM, "backingByteBuffer", "()L" + MappedObjectTransformer.jvmClassName(ByteBuffer.class) + ";"
            );
            mv.visitInsn(89);
            mv.visitMethodInsn(182, MappedObjectTransformer.jvmClassName(ByteBuffer.class), "capacity", "()I");
            mv.visitInsn(95);
            mv.visitMethodInsn(
               184, MappedObjectTransformer.jvmClassName(MemoryUtil.class), "getAddress0", "(L" + MappedObjectTransformer.jvmClassName(Buffer.class) + ";)J"
            );
            mv.visitVarInsn(25, 0);
            mv.visitFieldInsn(180, MappedObjectTransformer.MAPPED_OBJECT_JVM, "baseAddress", "J");
            mv.visitInsn(101);
         ...
         private void generateByteBufferGetter(String fieldName, MappedObjectTransformer.FieldInfo field) {
            MethodVisitor mv = super.visitMethod(
               9, MappedObjectTransformer.getterName(fieldName), "(L" + className + ";I)" + field.type.getDescriptor(), null, null
            );
            mv.visitCode();
            mv.visitVarInsn(25, 0);
            mv.visitVarInsn(21, 1);
            ...
            mv.visitMethodInsn(
               184, MappedObjectTransformer.MAPPED_HELPER_JVM, "newBuffer", "(JI)L" + MappedObjectTransformer.jvmClassName(ByteBuffer.class) + ";"
            );
            mv.visitInsn(176);
            mv.visitMaxs(3, 2);
            mv.visitEnd();
         ...
         private void generateFieldSetter(String fieldName, MappedObjectTransformer.FieldInfo field) {
            MethodVisitor mv = super.visitMethod(
               9, MappedObjectTransformer.setterName(fieldName), "(L" + className + ";I" + field.type.getDescriptor() + ")V", null, null
            );
            mv.visitCode();
            int load = 0;
   ...
   static int transformMethodCall(
      ...
      int i,
      Map<AbstractInsnNode, Frame<BasicValue>> frameMap,
      MethodInsnNode methodInsn,
      MappedObjectTransformer.MappedSubtypeInfo mappedType,
      Map<Integer, MappedObjectTransformer.MappedSubtypeInfo> arrayVars
   ) {
      switch (methodInsn.getOpcode()) {
         case 182:
            if ("asArray".equals(methodInsn.name) && methodInsn.desc.equals("()[L" + MAPPED_OBJECT_JVM + ";")) {
               ...
               checkInsnAfterIsArray(var11 = nextInstruction.getNext(), 58);
               Frame<BasicValue> frame = frameMap.get(var11);
               ...
               VarInsnNode varInstruction = (VarInsnNode)var11;
   ...
   private static InsnList generateMapInstructions(
      MappedObjectTransformer.MappedSubtypeInfo mappedType, String className, boolean mapDirectMethod, boolean mallocMethod
   ) {
      ...
      if (mallocMethod) {
         ...
         trg.add(
            new MethodInsnNode(
               184,
               mappedType.cacheLinePadded ? jvmClassName(CacheUtil.class) : jvmClassName(BufferUtils.class),
               "createByteBuffer",
               "(I)L" + jvmClassName(ByteBuffer.class) + ";"
            )
         );
   ...
   static InsnList transformFieldAccess(FieldInsnNode fieldInsn) {
      MappedObjectTransformer.MappedSubtypeInfo mappedSubtype = className_to_subtype.get(fieldInsn.owner);
      if (mappedSubtype == null) {
         ...
         MappedObjectTransformer.FieldInfo field = mappedSubtype.fields.get(fieldInsn.name);
         if (field == null) {
            return null;
         } else {
            return fieldInsn.desc.equals("L" + jvmClassName(ByteBuffer.class) + ";")
               ? generateByteBufferInstructions(fieldInsn, mappedSubtype, field.offset)
               : generateFieldInstructions(fieldInsn, field);
         }
      } else {
   ...
   static int transformArrayAccess(
      InsnList instructions,
      int i,
      Map<AbstractInsnNode, Frame<BasicValue>> frameMap,
      VarInsnNode loadInsn,
      MappedObjectTransformer.MappedSubtypeInfo mappedSubtype,
      int var
   ) {
                  ...
                  if (frame != null) {
                     ...
                     if (stackSize == loadStackSize + 1 && nextInsn.getOpcode() == 181) {
                        ...
                        instructions.insert(
                           nextInsn,
                           new MethodInsnNode(
                              184, mappedSubtype.className, setterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I" + fieldInsn.desc + ")V"
                           )
                        );
                     ...
                     if (stackSize == loadStackSize && nextInsn.getOpcode() == 180) {
                        ...
                        instructions.insert(
                           nextInsn,
                           new MethodInsnNode(184, mappedSubtype.className, getterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I)" + fieldInsn.desc)
                        );
                        instructions.remove(nextInsn);
                        break;
                     }
                     ...
                     if (stackSize == loadStackSize && nextInsn.getOpcode() == 89 && nextInsn.getNext().getOpcode() == 180) {
                        ...
                        MethodInsnNode getter = new MethodInsnNode(
                           184, mappedSubtype.className, getterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I)" + fieldInsn.desc
                        );
   ...
   static AbstractInsnNode getIntNode(int value) {
      if (value <= 5 && -1 <= value) {
         ...
         return (AbstractInsnNode)(value >= -32768 && value <= 32767 ? new IntInsnNode(17, value) : new LdcInsnNode(value));
      }
   }
   ...
   static {
      ...
      className_to_subtype.put(MAPPED_OBJECT_JVM, new MappedObjectTransformer.MappedSubtypeInfo(MAPPED_OBJECT_JVM, null, -1, -1, -1, false));
      String vmName = System.getProperty("java.vm.name");
      if (vmName != null && !vmName.contains("Server")) {
         System.err
            .println("Warning: " + MappedObject.class.getSimpleName() + "s have inferiour performance on Client VMs, please consider switching to a Server VM.");
      }
   }
   ...
   private static class TransformationAdapter extends ClassAdapter {
      ...
      public FieldVisitor visitField(int access, String name, String desc, String signature, Object value) {
         ...
         if (mappedSubtype != null && mappedSubtype.fields.containsKey(name)) {
            ...
            return (FieldVisitor)((access & 8) == 0
                  ...
                  public void visitEnd() {
                     if (this.visibleAnnotations == null) {
                           ...
                           if (MappedObjectTransformer.CACHE_LINE_PAD_JVM.equals(pad.desc)) {
                              if ("J".equals(this.desc) || "D".equals(this.desc)) {
                                 ...
                                 if (!"B".equals(this.desc) && !"Z".equals(this.desc)) {
                                    throw new ClassFormatError(
                                       "The @CacheLinePad annotation cannot be used on non-primitive fields: "
                                          + TransformationAdapter.this.className
                                          + "."
                                          + this.name
                                    );
                                 }
                              ...
                              if (pad.values != null) {
                                 for (int i = 0; i < pad.values.size(); i += 2) {
                                    boolean valuex = pad.values.get(i + 1).equals(Boolean.TRUE);
                        ...
                        if (before) {
                           ...
                           for (int ix = count; ix >= 1; ix--) {
                              TransformationAdapter.this.cv.visitField(this.access | 1 | 4096, this.name + "$PAD_" + ix, this.desc, this.signature, null);
                           }
                        }
                        ...
                        if (after) {
                           ...
                           for (int ix = 1; ix <= count; ix++) {
                              TransformationAdapter.this.cv.visitField(this.access | 1 | 4096, this.name + "$PAD" + ix, this.desc, this.signature, null);
                           }
                        }
                     }
                  }
      ...
      public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
         ...
         return new MethodNode(access, name, desc, signature, exceptions) {
            ...
            public void visitMethodInsn(int opcode, String owner, String namex, String descx) {
               if (MappedObjectTransformer.className_to_subtype.containsKey(owner)) {
                  this.needsTransformation = true;
               }

               super.visitMethodInsn(opcode, owner, namex, descx);
            }
                  ...
                  switch (instruction.getType()) {
                     case 2:
                        if (instruction.getOpcode() == 25) {
                           ...
                           MappedObjectTransformer.MappedSubtypeInfo mappedSubtype = arrayVars.get(varInsn.var);
                           if (mappedSubtype != null) {
                     ...
                     case 5:
                        ...
                        MappedObjectTransformer.MappedSubtypeInfo mappedType = MappedObjectTransformer.className_to_subtype.get(methodInsn.owner);
                        if (mappedType != null) {
