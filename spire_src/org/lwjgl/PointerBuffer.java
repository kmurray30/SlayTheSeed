...
public class PointerBuffer implements Comparable {
   ...
   public PointerBuffer slice() {
      ...
      ((Buffer)this.pointers).position(this.view.position() * pointerSize);
      ((Buffer)this.pointers).limit(this.view.limit() * pointerSize);

      PointerBuffer var2;
      try {
         var2 = this.newInstance(this.pointers);
      } finally {
         ((Buffer)this.pointers).clear();
      }
   ...
   public long get(int index) {
      return is64Bit ? this.view64.get(index) : this.view32.get(index) & 4294967295L;
   }
   ...
   public PointerBuffer get(long[] dst, int offset, int length) {
      if (is64Bit) {
         ...
         for (int i = offset; i < end; i++) {
            dst[i] = this.view32.get() & 4294967295L;
         }
      }
