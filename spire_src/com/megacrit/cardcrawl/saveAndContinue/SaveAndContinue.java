...
public class SaveAndContinue {
    ...
    public static boolean saveExistsAndNotCorrupted(AbstractPlayer p) {
        String filepath = SaveAndContinue.getPlayerSavePath(p.chosenClass);
        boolean fileExists = Gdx.files.local(filepath).exists();
        if (fileExists) {
            try {
                SaveAndContinue.loadSaveFile(filepath);
            }
            catch (SaveFileLoadError saveFileLoadError) {
                SaveAndContinue.deleteSave(p);
    ...
    public static String loadSaveString(AbstractPlayer.PlayerClass c) {
        return SaveAndContinue.loadSaveString(SaveAndContinue.getPlayerSavePath(c));
    }
    ...
    public static SaveFile loadSaveFile(AbstractPlayer.PlayerClass c) {
        String fileName = SaveAndContinue.getPlayerSavePath(c);
        try {
            return SaveAndContinue.loadSaveFile(fileName);
        }
    ...
    private static SaveFile loadSaveFile(String filePath) throws SaveFileLoadError {
        ...
        block4: {
            ...
            err = null;
            try {
                savestr = SaveAndContinue.loadSaveString(filePath);
                ...
                err = e;
                if (filePath.endsWith(".backUp")) break block4;
                logger.info(filePath + " was corrupt, loading backup...");
                return SaveAndContinue.loadSaveFile(filePath + ".backUp");
            }
        }
        if (saveFile == null) {
    ...
    public static void save(SaveFile save) {
        ...
        if (AbstractDungeon.player.hasRelic("Bottled Flame")) {
            SaveAndContinue.saveBottle(params, "Bottled Flame", "bottled_flame", ((BottledFlame)AbstractDungeon.player.getRelic((String)"Bottled Flame")).card);
        } else {
            params.put("bottled_flame", null);
        }
        if (AbstractDungeon.player.hasRelic("Bottled Lightning")) {
            SaveAndContinue.saveBottle(params, "Bottled Lightning", "bottled_lightning", ((BottledLightning)AbstractDungeon.player.getRelic((String)"Bottled Lightning")).card);
        } else {
            params.put("bottled_lightning", null);
        }
        if (AbstractDungeon.player.hasRelic("Bottled Tornado")) {
            SaveAndContinue.saveBottle(params, "Bottled Tornado", "bottled_tornado", ((BottledTornado)AbstractDungeon.player.getRelic((String)"Bottled Tornado")).card);
        } else {
