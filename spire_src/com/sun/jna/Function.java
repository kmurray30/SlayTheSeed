   ...
   Object invoke(Method invokingMethod, Class<?>[] paramTypes, Class<?> returnType, Object[] inArgs, Map<String, ?> options) {
      ...
      for (int i = 0; i < args.length; i++) {
         Class<?> paramType = invokingMethod != null
            ? (isVarArgs && i >= paramTypes.length - 1 ? paramTypes[paramTypes.length - 1].getComponentType() : paramTypes[i])
            : null;
   ...
   private Object convertArgument(Object[] args, int index, Method invokingMethod, TypeMapper mapper, boolean allowObjects, Class<?> expectedType) {
      ...
      if (arg != null && !this.isPrimitiveArray(arg.getClass())) {
         ...
         if (arg instanceof Structure) {
            ...
            Class<?> typex = argClass.getComponentType();
            boolean byRef = Structure.ByReference.class.isAssignableFrom(typex);
            if (expectedType != null && !Structure.ByReference[].class.isAssignableFrom(expectedType)) {
               if (byRef) {
                  throw new IllegalArgumentException(
                     "Function " + this.getName() + " declared Structure[] at parameter " + index + " but array of " + typex + " was passed"
                  );
               }
            ...
            if (byRef) {
               ...
               for (int ix = 0; ix < ss.length; ix++) {
                  pointers[ix] = ss[ix] != null ? ss[ix].getPointer() : null;
               }
               ...
               Structure.newInstance(typex).toArray(ss);
