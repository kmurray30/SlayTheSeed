...
public class CollectorStakeEffect extends AbstractGameEffect {
   ...
   public CollectorStakeEffect(float x, float y) {
      ...
      float randomAngle = (float) (Math.PI / 180.0) * MathUtils.random(-50.0F, 230.0F);
      ...
      this.targetAngle = MathUtils.atan2(y - this.y, x - this.x) * (180.0F / (float)Math.PI) + 90.0F;
      this.startingAngle = MathUtils.random(0.0F, 360.0F);
      this.rotation = this.startingAngle;
      this.x = this.x - img.packedWidth / 2;
      this.y = this.y - img.packedHeight / 2;
      this.sX = this.x;
      this.sY = this.y;
