   ...
   void encode(byte[] in, int inPos, int inAvail, BaseNCodec.Context context) {
      if (!context.eof) {
         if (inAvail < 0) {
            ...
            context.currentLinePos = context.currentLinePos + (context.pos - savedPos);
            if (this.lineLength > 0 && context.currentLinePos > 0) {
               System.arraycopy(this.lineSeparator, 0, buffer, context.pos, this.lineSeparator.length);
               context.pos = context.pos + this.lineSeparator.length;
            }
         } else {
   ...
   void decode(byte[] in, int inPos, int inAvail, BaseNCodec.Context context) {
      if (!context.eof) {
         ...
         if (context.eof && context.modulus != 0) {
            byte[] bufferx = this.ensureBufferSize(this.decodeSize, context);
            switch (context.modulus) {
               case 1:
                  break;
               case 2:
                  context.ibitWorkArea >>= 4;
                  bufferx[context.pos++] = (byte)(context.ibitWorkArea & 0xFF);
                  break;
               case 3:
                  context.ibitWorkArea >>= 2;
                  bufferx[context.pos++] = (byte)(context.ibitWorkArea >> 8 & 0xFF);
                  bufferx[context.pos++] = (byte)(context.ibitWorkArea & 0xFF);
                  break;
               default:
                  throw new IllegalStateException("Impossible modulus " + context.modulus);
