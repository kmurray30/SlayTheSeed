      ...
      switch (this.nextMove) {
            ...
            if (this.hb != null && AbstractDungeon.player.hb != null && !Settings.FAST_MODE) {
               AbstractDungeon.actionManager
                  .addToBottom(
                     new VFXAction(
                        new EntangleEffect(
                           this.hb.cX - 70.0F * Settings.scale, this.hb.cY + 10.0F * Settings.scale, AbstractDungeon.player.hb.cX, AbstractDungeon.player.hb.cY
                        ),
                        0.5F
                     )
                  );
         ...
         case 3:
            ...
            if (AbstractDungeon.ascensionLevel >= 17) {
               AbstractDungeon.actionManager
                  .addToBottom(
                     new ApplyPowerAction(AbstractDungeon.player, this, new VulnerablePower(AbstractDungeon.player, this.VULN_AMT + 1, true), this.VULN_AMT + 1)
                  );
            } else {
               AbstractDungeon.actionManager
                  .addToBottom(
                     new ApplyPowerAction(AbstractDungeon.player, this, new VulnerablePower(AbstractDungeon.player, this.VULN_AMT, true), this.VULN_AMT)
                  );
            }
      }
   ...
   protected void getMove(int num) {
      if (this.firstTurn) {
         ...
         if (!this.lastMove((byte)3)) {
            this.setMove(SCRAPE_NAME, (byte)3, AbstractMonster.Intent.ATTACK_DEBUFF, this.damage.get(1).base);
         } else {
            this.setMove((byte)1, AbstractMonster.Intent.ATTACK, this.damage.get(0).base);
         }
