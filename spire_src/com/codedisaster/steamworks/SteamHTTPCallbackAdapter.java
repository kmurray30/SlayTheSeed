...
class SteamHTTPCallbackAdapter extends SteamCallbackAdapter<SteamHTTPCallback> {
   ...
   void onHTTPRequestCompleted(long request, long contextValue, boolean requestSuccessful, int statusCode, int bodySize) {
      this.callback
         .onHTTPRequestCompleted(new SteamHTTPRequestHandle(request), contextValue, requestSuccessful, SteamHTTP.HTTPStatusCode.byValue(statusCode), bodySize);
   }

   void onHTTPRequestHeadersReceived(long request, long contextValue) {
      this.callback.onHTTPRequestHeadersReceived(new SteamHTTPRequestHandle(request), contextValue);
   }

   void onHTTPRequestDataReceived(long request, long contextValue, int offset, int bytesReceived) {
      this.callback.onHTTPRequestDataReceived(new SteamHTTPRequestHandle(request), contextValue, offset, bytesReceived);
   }
}
