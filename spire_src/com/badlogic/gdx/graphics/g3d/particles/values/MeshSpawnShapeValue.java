...
public abstract class MeshSpawnShapeValue extends SpawnShapeValue {
   ...
   public void load(AssetManager manager, ResourceData data) {
      ...
      if (descriptor != null) {
         ...
         this.setMesh(model.meshes.get(saveData.<Integer>load("index")), model);
      }
   }
   ...
   public static class Triangle {
      ...
      public Vector3 pick(Vector3 vector) {
         ...
         float b = MathUtils.random();
         return vector.set(
            this.x1 + a * (this.x2 - this.x1) + b * (this.x3 - this.x1),
            this.y1 + a * (this.y2 - this.y1) + b * (this.y3 - this.y1),
            this.z1 + a * (this.z2 - this.z1) + b * (this.z3 - this.z1)
         );
      }
   }
}
