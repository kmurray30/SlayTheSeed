...
public class SteamIntegration implements PublisherIntegration {
   ...
   static SteamIntegration.LeaderboardTask task = null;
   ...
   private static Queue<SteamIntegration.StatTuple> statsToUpload = new LinkedList<>();
   ...
   public int getStat(String id) {
      ...
      return steamStats != null ? steamStats.getStatI(id, 0) : -1;
   }
   ...
   public long getGlobalStat(String id) {
      ...
      return steamStats != null ? steamStats.getGlobalStat(id, 0L) : -1L;
   }
   ...
   public void getLeaderboardEntries(
      ...
      task = SteamIntegration.LeaderboardTask.RETRIEVE;
      startIndex = start;
      endIndex = end;
   ...
   public void getDailyLeaderboard(long date, int start, int end) {
      task = SteamIntegration.LeaderboardTask.RETRIEVE_DAILY;
      startIndex = start;
      endIndex = end;
      retrieveGlobal = true;
      gettingTime = false;
      if (steamStats != null) {
         ...
         steamStats.findOrCreateLeaderboard(
            leaderboardRetrieveString.toString(), SteamUserStats.LeaderboardSortMethod.Descending, SteamUserStats.LeaderboardDisplayType.Numeric
         );
      }
   }
   ...
   public void uploadLeaderboardScore(String name, int score) {
      if (steamUser != null && steamStats != null) {
         if (isUploadingScore) {
            statsToUpload.add(new SteamIntegration.StatTuple(name, score));
         } else {
            logger.info(String.format("Uploading Steam Leaderboard score (%s: %d)", name, score));
            isUploadingScore = true;
            task = SteamIntegration.LeaderboardTask.UPLOAD;
            lbScore = score;
   ...
   public void uploadDailyLeaderboardScore(String name, int score) {
      if (!TimeHelper.isOfflineMode()) {
         ...
         if (isUploadingScore) {
            ...
            isUploadingScore = true;
            task = SteamIntegration.LeaderboardTask.UPLOAD_DAILY;
            lbScore = score;
   ...
   void didCompleteCallback(boolean success) {
      ...
      if (statsToUpload.size() > 0) {
         SteamIntegration.StatTuple uploadMe = statsToUpload.remove();
   ...
   static void getLeaderboardEntryHelper() {
      if (task == SteamIntegration.LeaderboardTask.RETRIEVE) {
         if (retrieveGlobal) {
      ...
      } else if (task == SteamIntegration.LeaderboardTask.RETRIEVE_DAILY) {
   ...
   public void setRichPresenceDisplayPlaying(int floor, int ascension, String character) {
      ...
      if (Settings.isDailyRun) {
         ...
         && Settings.language != Settings.GameLanguage.THA
   ...
   public void setRichPresenceDisplayInMenu() {
      ...
      logger.debug("Setting Rich Presence: " + String.format(TEXT[3]));
