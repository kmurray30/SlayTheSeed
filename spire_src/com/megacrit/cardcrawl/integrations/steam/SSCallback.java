...
public class SSCallback implements SteamUserStatsCallback {
   ...
   public void onLeaderboardScoresDownloaded(SteamLeaderboardHandle handle, SteamLeaderboardEntriesHandle entries, int numEntries) {
      if (SteamIntegration.task == SteamIntegration.LeaderboardTask.RETRIEVE) {
               ...
               CardCrawlGame.mainMenuScreen
                  ...
                  .add(
                     new LeaderboardEntry(
                        rTemp,
                        SteamIntegration.steamFriends.getFriendPersonaName(entry.getSteamIDUser()),
                        entry.getScore(),
                        SteamIntegration.gettingTime,
                        SteamIntegration.accountId != -1 && SteamIntegration.accountId == entry.getSteamIDUser().getAccountID()
                     )
                  );
            } else {
         ...
         for (int ix = 0; ix < numEntries; ix++) {
            SteamLeaderboardEntry entry = new SteamLeaderboardEntry();
            if (SteamIntegration.steamStats.getDownloadedLeaderboardEntry(entries, ix, entry, details)) {
               int rTemp = entry.getGlobalRank();
               if (ix == 0) {
                  CardCrawlGame.mainMenuScreen.dailyScreen.currentStartIndex = rTemp;
               } else if (ix == numEntries) {
               ...
               logger.info("FAILED TO GET LEADERBOARD ENTRY: " + ix);
            }
         }
