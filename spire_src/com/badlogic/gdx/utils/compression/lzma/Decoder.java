...
public class Decoder {
   ...
   Decoder.LenDecoder m_LenDecoder = new Decoder.LenDecoder();
   Decoder.LenDecoder m_RepLenDecoder = new Decoder.LenDecoder();
   Decoder.LiteralDecoder m_LiteralDecoder = new Decoder.LiteralDecoder();
   ...
   public boolean Code(InputStream inStream, OutputStream outStream, long outSize) throws IOException {
      ...
      while (outSize < 0L || nowPos64 < outSize) {
         ...
         if (this.m_RangeDecoder.DecodeBit(this.m_IsMatchDecoders, (state << 4) + posState) == 0) {
            Decoder.LiteralDecoder.Decoder2 decoder2 = this.m_LiteralDecoder.GetDecoder((int)nowPos64, prevByte);
   ...
   class LiteralDecoder {
      ...
      public void Create(int numPosBits, int numPrevBits) {
         if (this.m_Coders == null || this.m_NumPrevBits != numPrevBits || this.m_NumPosBits != numPosBits) {
            ...
            for (int i = 0; i < numStates; i++) {
               this.m_Coders[i] = new Decoder.LiteralDecoder.Decoder2();
            }
         }
      }
      ...
      Decoder.LiteralDecoder.Decoder2 GetDecoder(int pos, byte prevByte) {
