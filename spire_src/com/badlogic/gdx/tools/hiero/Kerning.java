...
class Kerning {
   private Kerning.TTFInputStream input;
   private float scale;
   ...
   public void load(InputStream inputStream, int fontSize) throws IOException {
      if (inputStream == null) {
         ...
         this.input = new Kerning.TTFInputStream(inputStream);
         inputStream.close();
   ...
   private void storeKerningOffset(int firstGlyphCode, int secondGlyphCode, int offset) {
      int value = Math.round(offset * this.scale);
      if (value != 0) {
   ...
   private void readHEAD(int fontSize) throws IOException {
      ...
      this.scale = (float)fontSize / unitsPerEm;
   }
      ...
      for (int subTableCount = this.input.readUnsignedShort(); subTableCount > 0; subTableCount--) {
         ...
         if (tupleIndex >> 8 == 0) {
            ...
            while (kerningCount-- > 0) {
               ...
               int offset = this.input.readShort();
   ...
   private int readXAdvanceFromValueRecord(int valueFormat) throws IOException {
      ...
      for (int mask = 1; mask <= 32768 && mask <= valueFormat; mask <<= 1) {
         if ((valueFormat & mask) != 0) {
            int value = this.input.readShort();
            if (mask == 4) {
               xAdvance = value;
            }
         }
   ...
   private static class TTFInputStream extends ByteArrayInputStream {
      ...
      public long readUnsignedLong() throws IOException {
         ...
         value = (value << 8) + this.readUnsignedByte();
         return (value << 8) + this.readUnsignedByte();
      }
