...
final class LinuxDisplay implements DisplayImplementation {
   ...
   private static int globalErrorHandler(long display, long event_ptr, long error_display, long serial, long error_code, long request_code, long minor_code) throws LWJGLException {
      if (xembedded && request_code == 42L) {
         ...
         throw new LWJGLException(
            "X Error - disp: 0x"
               + Long.toHexString(error_display)
               + " serial: "
               + serial
               + " error: "
               + error_msg
         ...
         try {
            ...
            try {
               ...
               boolean undecorated = Display.getPrivilegedBoolean("org.lwjgl.opengl.Window.undecorated")
                  || current_window_mode != 3 && Display.getPrivilegedBoolean("org.lwjgl.opengl.Window.undecorated_fs");
               this.parent = parent;
               ...
               current_window = nCreateWindow(
                  getDisplay(), getDefaultScreen(), handle, mode, current_window_mode, x, y, undecorated, this.parent_window, this.resizable
               );
   ...
   private static native long nCreateWindow(
      long var0, int var2, ByteBuffer var3, DisplayMode var4, int var5, int var6, int var7, boolean var8, long var9, boolean var11
   ...
   private void processEvents() {
      while (LinuxEvent.getPending(getDisplay()) > 0) {
         ...
         if (event_window == getWindow()
            && !this.event_buffer.filterEvent(event_window)
            && (this.mouse == null || !this.mouse.filterEvent(this.grab, this.shouldWarpPointer(), this.event_buffer))
            && (this.keyboard == null || !this.keyboard.filterEvent(this.event_buffer))) {
   ...
   public Object createCursor(int width, int height, int xHotspot, int yHotspot, int numImages, IntBuffer images, IntBuffer delays) throws LWJGLException {
      ...
      try {
         ...
         try {
            long cursor = nCreateCursor(
               getDisplay(), width, height, xHotspot, yHotspot, numImages, images, images.position(), delays, delays != null ? delays.position() : -1
            );
            var10 = cursor;
   ...
   private static ByteBuffer convertIcons(ByteBuffer[] icons) {
      ...
      if (bufferSize == 0) {
         ...
         for (ByteBuffer iconx : icons) {
            int size = iconx.limit() / 4;
            int dimension = (int)Math.sqrt(size);
            ...
            for (int y = 0; y < dimension; y++) {
               for (int x = 0; x < dimension; x++) {
                  byte r = iconx.get(x * 4 + y * dimension * 4);
                  byte g = iconx.get(x * 4 + y * dimension * 4 + 1);
                  byte b = iconx.get(x * 4 + y * dimension * 4 + 2);
                  byte a = iconx.get(x * 4 + y * dimension * 4 + 3);
                  icon_argb.put(a);
                  icon_argb.put(r);
                  icon_argb.put(g);
                  icon_argb.put(b);
                           ...
                           if (LinuxDisplay.Compiz.isProcessActive("dbus-daemon")) {
                              ...
                              LinuxDisplay.Compiz.provider = new LinuxDisplay.Compiz.Provider() {
                                 ...
                                 public boolean hasLegacyFullscreenSupport() throws LWJGLException {
                                    List output = LinuxDisplay.Compiz.run(
                                       "dbus-send",
                                       "--print-reply",
                                       "--type=method_call",
                                       "--dest=org.freedesktop.compiz",
                                       "/org/freedesktop/compiz/workarounds/allscreens/legacy_fullscreen",
                                 ...
                                 public void setLegacyFullscreenSupport(boolean state) throws LWJGLException {
                                    if (LinuxDisplay.Compiz.run(
                                          "dbus-send",
                                          "--type=method_call",
                                          "--dest=org.freedesktop.compiz",
                                          "/org/freedesktop/compiz/workarounds/allscreens/legacy_fullscreen",
                                          "org.freedesktop.compiz.set",
                                          "boolean:" + Boolean.toString(state)
                                       )
                                       == null) {
