...
public class Pointer {
   ...
   public static final Pointer createConstant(int peer) {
      return new Pointer.Opaque(peer & -1L);
   }
   ...
   public void read(long offset, Pointer[] buf, int index, int length) {
      for (int i = 0; i < length; i++) {
         Pointer p = this.getPointer(offset + i * SIZE);
   ...
   private void readArray(long offset, Object o, Class<?> cls) {
      ...
      if (cls == byte.class) {
         ...
         for (int ix = 0; ix < array.length; ix++) {
            Object value = this.getValue(offset + size * ix, tc.nativeType(), array[ix]);
            array[ix] = (NativeMapped)tc.fromNative(value, new FromNativeContext(cls));
         }
      }
   }
   ...
   private void writeArray(long offset, Object value, Class<?> cls) {
      if (cls == byte.class) {
         ...
         if (Structure.ByReference.class.isAssignableFrom(cls)) {
            ...
            for (int ix = 1; ix < sbuf.length; ix++) {
               if (sbuf[ix] == null) {
                  sbuf[ix] = tmp[ix];
               } else {
                  sbuf[ix].useMemory(this, (int)(offset + ix * sbuf[ix].size()), true);
               }
         ...
         for (int ix = 0; ix < buf.length; ix++) {
            Object element = tc.toNative(buf[ix], new ToNativeContext());
            this.setValue(offset + ix * size, element, nativeType);
         }
      }
   }
