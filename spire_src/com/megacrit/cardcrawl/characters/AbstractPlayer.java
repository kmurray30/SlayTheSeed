...
public abstract class AbstractPlayer extends AbstractCreature {
   ...
   public void initializeStarterDeck() {
      ...
      if (ModHelper.isModEnabled("Draft")
         || ModHelper.isModEnabled("Chimera")
         || ModHelper.isModEnabled("SealedDeck")
         || ModHelper.isModEnabled("Shiny")
         || ModHelper.isModEnabled("Insanity")) {
         addBaseCards = false;
      }
               ...
               if (!orbHovered || !CInputActionSet.right.isJustPressed() && !CInputActionSet.altRight.isJustPressed()) {
                  if (!this.inspectMode || !CInputActionSet.down.isJustPressed() && !CInputActionSet.altDown.isJustPressed()) {
                     if (!this.inspectMode
                        ...
                        for (AbstractMonster mxx : AbstractDungeon.getMonsters().monsters) {
                           if (!mxx.isDying && !mxx.isEscaping) {
                              hbs.add(mxx.hb);
                           }
                        }
   ...
   public boolean isCursed() {
      ...
      for (AbstractCard c : this.masterDeck.group) {
         if (c.type == AbstractCard.CardType.CURSE && c.cardID != "Necronomicurse" && c.cardID != "CurseOfTheBell" && c.cardID != "AscendersBane") {
            cursed = true;
         }
      }
   ...
   public void updateInput() {
      if (!this.viewingRelics) {
         if (this.hoverEnemyWaitTimer > 0.0F) {
            this.hoverEnemyWaitTimer = this.hoverEnemyWaitTimer - Gdx.graphics.getDeltaTime();
         }
         ...
         if (this.inSingleTargetMode) {
            ...
            if (!Settings.isTouchScreen) {
               this.isHoveringDropZone = (y > this.hoverStartLine || y > 300.0F * Settings.scale) && y < Settings.CARD_DROP_END_Y
                  || hMonster
   ...
   private boolean updateFullKeyboardCardSelection() {
      ...
      if (!this.isInKeyboardMode || this.hand.isEmpty() || this.inspectMode) {
         return false;
      } else if (this.keyboardCardIndex == -2) {
   ...
   private void renderCardHotKeyText(SpriteBatch sb) {
      ...
      for (AbstractCard card : this.hand.group) {
         if (index < InputActionSet.selectCardActions.length) {
            ...
            float sin = (float)Math.sin(card.angle / 180.0F * Math.PI);
            float xOffset = sin * width;
            FontHelper.renderFontCentered(
               sb, FontHelper.buttonLabelFont, InputActionSet.selectCardActions[index].getKeyString(), card.current_x - xOffset, textY, Settings.CREAM_COLOR
            );
         }
   ...
   private boolean clickAndDragCards() {
      ...
      if (CInputActionSet.select.isJustPressed() && this.hoveredCard != null && !this.isCardQueued(this.hoveredCard) && !this.isDraggingCard) {
         ...
         if (InputHelper.isMouseDown) {
            this.clickDragTimer = this.clickDragTimer + Gdx.graphics.getDeltaTime();
         } else {
         ...
         if ((InputHelper.justClickedLeft || InputActionSet.confirm.isJustPressed() || CInputActionSet.select.isJustPressed()) && this.isUsingClickDragControl) {
            ...
            if (!this.isDraggingCard || !InputHelper.isMouseDown && !this.isUsingClickDragControl) {
               ...
               if (!this.hoveredCard.hasEnoughEnergy() && this.isHoveringDropZone) {
                  ...
                  if (this.isHoveringDropZone
                     && (this.hoveredCard.target == AbstractCard.CardTarget.ENEMY || this.hoveredCard.target == AbstractCard.CardTarget.SELF_AND_ENEMY)) {
                     this.inSingleTargetMode = true;
                     this.arrowX = InputHelper.mX;
                     this.arrowY = InputHelper.mY;
                     GameCursor.hidden = true;
   ...
   public int getCircletCount() {
      ...
      return counterSum > 0 ? counterSum : count;
   }
      ...
      if (!tips.isEmpty()) {
         if (this.hb.cX + this.hb.width / 2.0F < TIP_X_THRESHOLD) {
            TipHelper.queuePowerTips(
               this.hb.cX + this.hb.width / 2.0F + TIP_OFFSET_R_X, this.hb.cY + TipHelper.calculateAdditionalOffset(tips, this.hb.cY), tips
            );
         } else {
            TipHelper.queuePowerTips(
               this.hb.cX - this.hb.width / 2.0F + TIP_OFFSET_L_X, this.hb.cY + TipHelper.calculateAdditionalOffset(tips, this.hb.cY), tips
            );
         }
      }
   }
   ...
   private void drawCurvedLine(SpriteBatch sb, Vector2 start, Vector2 end, Vector2 control) {
      ...
      for (int i = 0; i < this.points.length - 1; i++) {
         this.points[i] = Bezier.quadratic(this.points[i], i / 20.0F, start, control, end, this.arrowTmp);
         ...
         if (i != 0) {
            ...
            sb.draw(
               ...
               radius / 18.0F,
               radius / 18.0F,
               this.arrowTmp.nor().angle() + 270.0F,
               0,
               0,
               128,
               128,
   ...
   public void applyStartOfTurnCards() {
      ...
      for (AbstractCard cx : this.hand.group) {
         if (cx != null) {
            cx.atTurnStart();
         }
      }

      for (AbstractCard cxx : this.discardPile.group) {
         if (cxx != null) {
            cxx.atTurnStart();
         }
      }
   ...
   public boolean hasOrb() {
      return this.orbs.isEmpty() ? false : !(this.orbs.get(0) instanceof EmptyOrbSlot);
   }
   ...
   private void updateEscapeAnimation() {
      if (this.escapeTimer != 0.0F) {
         ...
         if (this.flipHorizontal) {
            ...
            this.drawX = this.drawX + Gdx.graphics.getDeltaTime() * 500.0F * Settings.scale;
         }
      }
