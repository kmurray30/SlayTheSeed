...
public final class LinuxEnvironmentPlugin extends ControllerEnvironment implements Plugin {
   ...
   private static final Component[] createComponents(List event_components, LinuxEventDevice device) {
      ...
      for (int ix = 0; ix < povs.length; ix++) {
         LinuxEventComponent x = povs[ix][0];
         LinuxEventComponent y = povs[ix][1];
         if (x != null && y != null) {
   ...
   private static final Controller createJoystickFromJoystickDevice(LinuxJoystickDevice device) {
      ...
      for (int ix = 0; ix < device.getNumAxes(); ix++) {
         Component.Identifier.Axis axis_id = (Component.Identifier.Axis)LinuxNativeTypesMap.getAbsAxisID(axisMap[ix]);
         LinuxJoystickAxis axis = new LinuxJoystickAxis(axis_id);
         device.registerAxis(ix, axis);
         if (axisMap[ix] == 16) {
            hatBits[0] = axis;
         } else if (axisMap[ix] == 17) {
            hatBits[1] = axis;
         ...
         } else if (axisMap[ix] == 18) {
            hatBits[2] = axis;
         } else if (axisMap[ix] == 19) {
            hatBits[3] = axis;
         ...
         } else if (axisMap[ix] == 20) {
            hatBits[4] = axis;
         } else if (axisMap[ix] == 21) {
            hatBits[5] = axis;
