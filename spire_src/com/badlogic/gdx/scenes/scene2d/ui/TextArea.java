...
public class TextArea extends TextField {
   ...
   protected int letterUnderCursor(float x) {
      if (this.linesBreak.size <= 0) {
         ...
         return glyphPositions[i] - x <= x - glyphPositions[i - 1] ? i : Math.max(0, i - 1);
      }
   }
   ...
   protected void drawSelection(Drawable selection, Batch batch, BitmapFont font, float x, float y) {
      ...
      for (int maxIndex = Math.max(this.cursor, this.selectionStart);
         i + 1 < this.linesBreak.size && i < (this.firstLineShowing + this.linesShowing) * 2;
         i += 2
      ) {
   ...
   protected void drawCursor(Drawable cursorPatch, Batch batch, BitmapFont font, float x, float y) {
      float textOffset = this.cursor < this.glyphPositions.size && this.cursorLine * 2 < this.linesBreak.size
         ? this.glyphPositions.get(this.cursor) - this.glyphPositions.get(this.linesBreak.items[this.cursorLine * 2])
         : 0.0F;
      cursorPatch.draw(
         batch,
         x + textOffset + this.fontOffset + font.getData().cursorX,
         y - font.getDescent() / 2.0F - (this.cursorLine - this.firstLineShowing + 1) * font.getLineHeight(),
