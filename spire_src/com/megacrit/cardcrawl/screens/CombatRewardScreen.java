...
public class CombatRewardScreen {
   ...
   public void setupItemReward() {
      ...
      if ((AbstractDungeon.getCurrRoom().event == null || AbstractDungeon.getCurrRoom().event != null && !AbstractDungeon.getCurrRoom().event.noCardsInRewards)
         ...
         } else if (AbstractDungeon.getCurrRoom() instanceof MonsterRoomElite || AbstractDungeon.getCurrRoom() instanceof MonsterRoomBoss) {
            RewardItem cardRewardx = new RewardItem();
            if (cardRewardx.cards.size() > 0) {
               this.rewards.add(cardRewardx);
            }
         }
      }
   ...
   public void positionRewards() {
      for (int i = 0; i < this.rewards.size(); i++) {
         this.rewards.get(i).move(Settings.HEIGHT / 2.0F + 124.0F * Settings.scale - i * 100.0F * Settings.scale);
      }
   ...
   private void rewardViewUpdate() {
      if (this.rewardAnimTimer != 0.0F) {
         this.rewardAnimTimer = this.rewardAnimTimer - Gdx.graphics.getDeltaTime();
         if (this.rewardAnimTimer < 0.0F) {
   ...
   private void updateControllerInput() {
      if (Settings.isControllerMode
         ...
         if (!anyHovered) {
            int var5 = 0;
            Gdx.input.setCursorPosition((int)this.rewards.get(var5).hb.cX, Settings.HEIGHT - (int)this.rewards.get(var5).hb.cY);
         } else if (CInputActionSet.up.isJustPressed() || CInputActionSet.altUp.isJustPressed()) {
            if (--index < 0) {
               index = this.rewards.size() - 1;
            }

            Gdx.input.setCursorPosition((int)this.rewards.get(index).hb.cX, Settings.HEIGHT - (int)this.rewards.get(index).hb.cY);
   ...
   private void setLabel() {
      if (this.rewards.size() == 0) {
         ...
         switch (this.rewards.get(0).type) {
            case CARD:
   ...
   public void render(SpriteBatch sb) {
      ...
      FontHelper.renderFontCentered(sb, FontHelper.panelNameFont, TEXT[12] + this.tip, Settings.WIDTH / 2.0F, this.tipY, Color.LIGHT_GRAY);
   ...
   private void renderItemReward(SpriteBatch sb) {
      ...
      sb.draw(
         ...
         Settings.WIDTH / 2.0F - 306.0F,
         Settings.HEIGHT / 2.0F - 46.0F * Settings.scale - 358.0F,
         306.0F,
         358.0F,
         612.0F,
         716.0F,
