...
public class BoundingBox implements Serializable {
   ...
   public BoundingBox ext(Vector3 point) {
      return this.set(
         this.min.set(min(this.min.x, point.x), min(this.min.y, point.y), min(this.min.z, point.z)),
   ...
   public BoundingBox ext(BoundingBox a_bounds) {
      return this.set(
         this.min.set(min(this.min.x, a_bounds.min.x), min(this.min.y, a_bounds.min.y), min(this.min.z, a_bounds.min.z)),
         this.max.set(max(this.max.x, a_bounds.max.x), max(this.max.y, a_bounds.max.y), max(this.max.z, a_bounds.max.z))
      );
   }

   public BoundingBox ext(Vector3 center, float radius) {
      return this.set(
         this.min.set(min(this.min.x, center.x - radius), min(this.min.y, center.y - radius), min(this.min.z, center.z - radius)),
         this.max.set(max(this.max.x, center.x + radius), max(this.max.y, center.y + radius), max(this.max.z, center.z + radius))
      );
   }
   ...
   public boolean contains(BoundingBox b) {
      return !this.isValid()
         || this.min.x <= b.min.x && this.min.y <= b.min.y && this.min.z <= b.min.z && this.max.x >= b.max.x && this.max.y >= b.max.y && this.max.z >= b.max.z;
   }
   ...
   public BoundingBox ext(float x, float y, float z) {
      return this.set(
         this.min.set(min(this.min.x, x), min(this.min.y, y), min(this.min.z, z)), this.max.set(max(this.max.x, x), max(this.max.y, y), max(this.max.z, z))
      );
   }
