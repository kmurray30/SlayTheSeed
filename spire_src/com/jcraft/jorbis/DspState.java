...
public class DspState {
      ...
      switch (type) {
         case 0:
            ...
            for (int i = 0; i < left; i++) {
               float x = (float)((i + 0.5) / left * (float) Math.PI / 2.0);
               x = (float)Math.sin(x);
               x *= x;
               x = (float)(x * (float) (Math.PI / 2));
               x = (float)Math.sin(x);
               ret[i + leftbegin] = x;
            }
            ...
            for (int i = 0; i < right; i++) {
               float x = (float)((right - i - 0.5) / right * (float) Math.PI / 2.0);
               x = (float)Math.sin(x);
               x *= x;
               x = (float)(x * (float) (Math.PI / 2));
               x = (float)Math.sin(x);
   ...
   public int synthesis_init(Info vi) {
      ...
      this.centerW = this.centerW - (vi.blocksizes[this.W] / 4 + vi.blocksizes[this.lW] / 4);
      this.granulepos = -1L;
      this.sequence = -1L;
      return 0;
   }
   ...
   public int synthesis_blockin(Block vb) {
      ...
      this.W = vb.W;
      this.nW = -1;
      this.glue_bits = this.glue_bits + vb.glue_bits;
      this.time_bits = this.time_bits + vb.time_bits;
      this.floor_bits = this.floor_bits + vb.floor_bits;
      this.res_bits = this.res_bits + vb.res_bits;
      ...
      if (this.granulepos == -1L) {
         ...
         this.granulepos = this.granulepos + (_centerW - this.centerW);
