   ...
   public AbstractCard.CardRarity getCardRarity(int roll, boolean useAlternation) {
      ...
      if (roll < this.rareCardChance) {
         ...
         if (roll >= this.baseRareCardChance + this.baseUncommonCardChance) {
            for (AbstractRelic rx : AbstractDungeon.player.relics) {
               if (rx.changeUncommonCardRewardChance(this.baseUncommonCardChance) > this.baseUncommonCardChance) {
                  rx.flash();
               }
            }
         }
         ...
         case COMBAT:
            ...
            if (!(waitTimer > 0.0F)) {
               ...
               if (AbstractDungeon.actionManager.currentAction == null && AbstractDungeon.actionManager.isEmpty()) {
                  waitTimer = waitTimer - Gdx.graphics.getDeltaTime();
               } else {
                  AbstractDungeon.actionManager.update();
               }
            ...
            if (this.isBattleOver && AbstractDungeon.actionManager.actions.isEmpty()) {
               this.skipMonsterTurn = false;
               this.endBattleTimer = this.endBattleTimer - Gdx.graphics.getDeltaTime();
               if (this.endBattleTimer < 0.0F) {
                  ...
                  if (this instanceof MonsterRoomBoss && !AbstractDungeon.loading_post_combat) {
                     if (!CardCrawlGame.loadingSave) {
                        if (Settings.isDailyRun) {
                           ...
                           if (AbstractDungeon.ascensionLevel >= 13) {
                              this.addGoldToRewards(MathUtils.round(tmp * 0.75F));
                           } else {
                     ...
                     if (ModHelper.isModEnabled("Cursed Run")) {
                        AbstractDungeon.effectList
                           .add(new ShowCardAndObtainEffect(AbstractDungeon.returnRandomCurse(), Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F));
                     }
                  ...
                  if (!(AbstractDungeon.getCurrRoom() instanceof MonsterRoomBoss)
                     || !(CardCrawlGame.dungeon instanceof TheBeyond) && !(CardCrawlGame.dungeon instanceof TheEnding)
                     || Settings.isEndless) {
         ...
         case COMPLETE:
            if (!AbstractDungeon.isScreenUp) {
               ...
               if (AbstractDungeon.actionManager.isEmpty() && !AbstractDungeon.isFadingOut) {
                  ...
                  this.rewardPopOutTimer = this.rewardPopOutTimer - Gdx.graphics.getDeltaTime();
   ...
   public void renderAboveTopPanel(SpriteBatch sb) {
      ...
      if (Settings.isInfo) {
         String msg = "[GAME MODE DATA]\n isDaily: "
            ...
            + "\n Particle Count: "
            + AbstractDungeon.effectList.size();
         ...
         if (AbstractDungeon.getCurrRoom().event != null
            ...
            if (!RoomEventDialog.optionList.isEmpty()) {
               ...
               if (!anyHovered) {
                  ...
                  Gdx.input
                     .setCursorPosition((int)RoomEventDialog.optionList.get(index).hb.cX, Settings.HEIGHT - (int)RoomEventDialog.optionList.get(index).hb.cY);
               ...
               if (!anyHovered) {
                  Gdx.input
                     .setCursorPosition(
                        (int)this.event.imageEventText.optionList.get(0).hb.cX, Settings.HEIGHT - (int)this.event.imageEventText.optionList.get(0).hb.cY
                     );
               } else if (CInputActionSet.down.isJustPressed() || CInputActionSet.altDown.isJustPressed()) {
                  ...
                  Gdx.input
                     .setCursorPosition(
                        (int)this.event.imageEventText.optionList.get(index).hb.cX,
                        Settings.HEIGHT - (int)this.event.imageEventText.optionList.get(index).hb.cY
                     );
               } else if (CInputActionSet.up.isJustPressed() || CInputActionSet.altUp.isJustPressed()) {
                  if (--index < 0) {
