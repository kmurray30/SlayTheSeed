...
final class CacheLineSize {
   ...
   static int getCacheLineSize() {
      ...
      double TIME_THRESHOLD = 1.0 + LWJGLUtil.getPrivilegedInteger("org.lwjgl.util.mapped.CacheLineTimeThreshold", 50).intValue() / 100.0;
      ...
      try {
         ...
         for (int i = MAX_SIZE; i >= 1; i >>= 1) {
            ...
            if (totalTime > 0L) {
               ...
               if ((double)time / avgTime > TIME_THRESHOLD) {
                  cacheLineSize = (i << 1) * 4;
                  found = true;
                  break;
               }
   ...
   private static IntBuffer getMemory(int START_SIZE) {
      ...
      if (MemoryUtil.getAddress(buffer) % PAGE_SIZE != 0L) {
         ((Buffer)buffer).position(PAGE_SIZE - (int)(MemoryUtil.getAddress(buffer) & PAGE_SIZE - 1));
      }
   ...
   private static void submitTest(
      ExecutorCompletionService<Long> completionService, final int index, final int repeats, final IntBuffer memory, final int padding
   ) {
