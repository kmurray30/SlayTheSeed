         ...
         case 3:
            ...
            AbstractDungeon.actionManager
               .addToBottom(
                  new VFXAction(
                     new BiteEffect(
                        AbstractDungeon.player.hb.cX + MathUtils.random(-25.0F, 25.0F) * Settings.scale,
                        AbstractDungeon.player.hb.cY + MathUtils.random(-25.0F, 25.0F) * Settings.scale,
                        Color.GOLD.cpy()
                     ),
                     0.0F
                  )
            ...
            AbstractDungeon.actionManager
               .addToBottom(new VampireDamageAction(AbstractDungeon.player, this.damage.get(2), AbstractGameAction.AttackEffect.NONE));
            break;
         case 4:
            this.setMove((byte)1, AbstractMonster.Intent.ATTACK_DEBUFF, this.damage.get(1).base);
      ...
      if (this.firstMove) {
         ...
         if (AbstractDungeon.ascensionLevel >= 17) {
            ...
            if (AbstractDungeon.aiRng.randomBoolean()) {
               this.setMove((byte)2, AbstractMonster.Intent.ATTACK, this.damage.get(0).base, 2, true);
            } else {
               this.setMove((byte)3, AbstractMonster.Intent.ATTACK_BUFF, this.damage.get(2).base);
            }
         }
      } else {
         if (num < 20) {
