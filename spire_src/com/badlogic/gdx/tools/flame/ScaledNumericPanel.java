...
class ScaledNumericPanel extends ParticleValuePanel<ScaledNumericValue> {
   ...
   private void initializeComponents(String chartTitle) {
      ...
      contentPanel.add(this.expandButton, new GridBagConstraints(7, 5, 1, 1, 1.0, 0.0, 16, 0, new Insets(0, 5, 0, 0), 0, 0));
      this.expandButton.setBorder(BorderFactory.createEmptyBorder(4, 8, 4, 8));
      this.relativeCheckBox = new JCheckBox("Relative");
      contentPanel.add(this.relativeCheckBox, new GridBagConstraints(7, 5, 1, 1, 0.0, 0.0, 18, 0, new Insets(0, 6, 0, 0), 0, 0));
      this.lowMinSlider.addChangeListener(new ChangeListener() {
         @Override
         public void stateChanged(ChangeEvent event) {
            ScaledNumericPanel.this.value.setLowMin(ScaledNumericPanel.this.lowMinSlider.getValue());
            if (!ScaledNumericPanel.this.lowMaxSlider.isVisible()) {
               ScaledNumericPanel.this.value.setLowMax(ScaledNumericPanel.this.lowMinSlider.getValue());
            }
         }
      });
      this.lowMaxSlider.addChangeListener(new ChangeListener() {
         @Override
         public void stateChanged(ChangeEvent event) {
            ScaledNumericPanel.this.value.setLowMax(ScaledNumericPanel.this.lowMaxSlider.getValue());
         }
      });
      this.highMinSlider.addChangeListener(new ChangeListener() {
         @Override
         public void stateChanged(ChangeEvent event) {
            ScaledNumericPanel.this.value.setHighMin(ScaledNumericPanel.this.highMinSlider.getValue());
            if (!ScaledNumericPanel.this.highMaxSlider.isVisible()) {
               ScaledNumericPanel.this.value.setHighMax(ScaledNumericPanel.this.highMinSlider.getValue());
            }
         }
      });
      this.highMaxSlider.addChangeListener(new ChangeListener() {
         @Override
         public void stateChanged(ChangeEvent event) {
            ScaledNumericPanel.this.value.setHighMax(ScaledNumericPanel.this.highMaxSlider.getValue());
         }
      });
      this.relativeCheckBox.addActionListener(new ActionListener() {
         @Override
         public void actionPerformed(ActionEvent event) {
            ScaledNumericPanel.this.value.setRelative(ScaledNumericPanel.this.relativeCheckBox.isSelected());
         }
      });
   ...
   public void setValue(ScaledNumericValue value) {
      ...
      if (this.value != null) {
         setValue(this.lowMinSlider, this.value.getLowMin());
         setValue(this.lowMaxSlider, this.value.getLowMax());
         setValue(this.highMinSlider, this.value.getHighMin());
         setValue(this.highMaxSlider, this.value.getHighMax());
         this.chart.setValues(this.value.getTimeline(), this.value.getScaling());
         setValue(this.relativeCheckBox, this.value.isRelative());
         if (this.value.getLowMin() == this.value.getLowMax() && this.lowMaxSlider.isVisible()
            || this.value.getLowMin() != this.value.getLowMax() && !this.lowMaxSlider.isVisible()) {
            this.lowRangeButton.doClick(0);
         }

         if (this.value.getHighMin() == this.value.getHighMax() && this.highMaxSlider.isVisible()
            || this.value.getHighMin() != this.value.getHighMax() && !this.highMaxSlider.isVisible()) {
