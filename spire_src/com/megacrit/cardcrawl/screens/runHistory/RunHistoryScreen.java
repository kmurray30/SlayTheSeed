...
public class RunHistoryScreen implements DropdownMenuListener {
   ...
   private static final AbstractCard.CardRarity[] orderedRarity = new AbstractCard.CardRarity[]{
   ...
   private static final AbstractRelic.RelicTier[] orderedRelicRarity = new AbstractRelic.RelicTier[]{
   ...
   public RunHistoryScreen() {
      ...
      this.nextHb.move(Settings.WIDTH - 180.0F * Settings.xScale, Settings.HEIGHT / 2.0F);
   }
   ...
   private void reloadRelics(RunData runData) {
      ...
      for (String relicName : runData.relics) {
         try {
            ...
            int newCount = relicRarityCounts.containsKey(relic.tier) ? relicRarityCounts.get(relic.tier) + 1 : 1;
   ...
   private void reloadCards(RunData runData) {
      ...
      for (AbstractCard cardx : rawNameToCards.values()) {
         sortedMasterDeck.addToTop(cardx);
      }
      ...
      for (AbstractCard cardx : sortedMasterDeck.group) {
         this.cards.add(new TinyCard(cardx, cardCounts.get(cardx)));
      }
   ...
   public void update() {
      ...
      if (Settings.isControllerMode && !CardCrawlGame.isPopupOpen && this.currentHb != null) {
         if (Gdx.input.getY() > Settings.HEIGHT * 0.8F) {
            this.scrollTargetY = this.scrollTargetY + Settings.SCROLL_SPEED / 2.0F;
      ...
      if (Settings.isControllerMode && !this.runsDropdown.isOpen && !this.winLossFilter.isOpen && !this.characterFilter.isOpen && !this.runTypeFilter.isOpen) {
         ...
         if (!anyHovered) {
            ...
            switch (section) {
               ...
               case ROOM:
                  if (CInputActionSet.up.isJustPressed() || CInputActionSet.altUp.isJustPressed()) {
                     int cx = this.runPath.pathElements.get(index).col;
                     int rx = this.runPath.pathElements.get(index).row - 1;
                     if (rx < 0) {
                        CInputHelper.setCursor(hbs.get(0));
                        this.currentHb = hbs.get(0);
                        this.scrollTargetY = 0.0F;
                        ...
                        for (int ixxxx = 0; ixxxx < this.runPath.pathElements.size(); ixxxx++) {
                           if (this.runPath.pathElements.get(ixxxx).row == rx && this.runPath.pathElements.get(ixxxx).col == cx) {
                              CInputHelper.setCursor(this.runPath.pathElements.get(ixxxx).hb);
                              this.currentHb = this.runPath.pathElements.get(ixxxx).hb;
                              foundNode = true;
                              break;
                           }
                        }
                        ...
                        if (!foundNode) {
                           for (int ixxxxx = this.runPath.pathElements.size() - 1; ixxxxx > 0; ixxxxx--) {
                              if (this.runPath.pathElements.get(ixxxxx).row == rx) {
                                 CInputHelper.setCursor(this.runPath.pathElements.get(ixxxxx).hb);
                                 this.currentHb = this.runPath.pathElements.get(ixxxxx).hb;
                                 foundNode = true;
                                 break;
                              }
                           }
                     ...
                     int cx = this.runPath.pathElements.get(index).col;
                     int rx = this.runPath.pathElements.get(index).row + 1;
                     if (rx > this.runPath.pathElements.get(this.runPath.pathElements.size() - 1).row) {
                        ...
                        for (int ixxxxxx = this.runPath.pathElements.size() - 1; ixxxxxx > 0; ixxxxxx--) {
                           if (this.runPath.pathElements.get(ixxxxxx).row == rx && this.runPath.pathElements.get(ixxxxxx).col == cx) {
                              CInputHelper.setCursor(this.runPath.pathElements.get(ixxxxxx).hb);
                              this.currentHb = this.runPath.pathElements.get(ixxxxxx).hb;
                              foundNode = true;
                              break;
                           }
                        }
                        ...
                        if (!foundNode) {
                           for (int ixxxxxxx = this.runPath.pathElements.size() - 1; ixxxxxxx > 0; ixxxxxxx--) {
                              if (this.runPath.pathElements.get(ixxxxxxx).row == rx) {
                                 CInputHelper.setCursor(this.runPath.pathElements.get(ixxxxxxx).hb);
                                 this.currentHb = this.runPath.pathElements.get(ixxxxxxx).hb;
                                 foundNode = true;
                                 break;
                              }
                           }
               ...
               case RELIC:
                  ...
                  break;
               case CARD:
                  int c = this.cards.get(index).col;
                  int r = this.cards.get(index).row;
                  if (CInputActionSet.left.isJustPressed() || CInputActionSet.altLeft.isJustPressed()) {
                     if (--c < 0) {
                        ...
                        for (int ix = 0; ix < this.cards.size(); ix++) {
                           if (this.cards.get(ix).col == c && this.cards.get(ix).row == r) {
                              CInputHelper.setCursor(this.cards.get(ix).hb);
                              this.currentHb = this.cards.get(ix).hb;
                              foundNode = true;
                              break;
                           }
                        }
                     ...
                     if (++c > TinyCard.desiredColumns - 1) {
                        ...
                        for (int ixx = 0; ixx < this.cards.size(); ixx++) {
                           if (this.cards.get(ixx).col == c && this.cards.get(ixx).row == r) {
                              CInputHelper.setCursor(this.cards.get(ixx).hb);
                              this.currentHb = this.cards.get(ixx).hb;
                              foundNode = true;
                              break;
                           }
                        }
                        ...
                        if (!foundNode) {
                           ...
                           for (int ixxx = 0; ixxx < this.cards.size(); ixxx++) {
                              if (this.cards.get(ixxx).col == var26 && this.cards.get(ixxx).row == r) {
                                 CInputHelper.setCursor(this.cards.get(ixxx).hb);
                                 this.currentHb = this.cards.get(ixxx).hb;
                                 break;
                              }
                           }
                        }
   ...
   private String characterText(String chosenCharacter) {
      ...
      } else if (chosenCharacter.equals(AbstractPlayer.PlayerClass.THE_SILENT.name())) {
         return SILENT_NAME;
      } else if (chosenCharacter.equals(AbstractPlayer.PlayerClass.DEFECT.name())) {
         return DEFECT_NAME;
      } else {
         return chosenCharacter.equals(AbstractPlayer.PlayerClass.WATCHER.name()) ? WATCHER_NAME : chosenCharacter;
      }
   }
   ...
   private void renderRunHistoryScreen(SpriteBatch sb) {
      ...
      if (this.viewedRun == null) {
         ...
         if (this.viewedRun.victory) {
            sb.setColor(Color.WHITE);
            sb.draw(
               ImageMaster.TIMER_ICON,
               header1x + this.approximateSubHeaderWidth(resultText) - 32.0F + 54.0F * Settings.scale,
               yOffset - 32.0F - 10.0F * Settings.scale,
               32.0F,
               32.0F,
               64.0F,
               64.0F,
            ...
            this.renderSubHeading(
               sb,
               CharStat.formatHMSM((float)this.viewedRun.playtime),
               header1x + this.approximateSubHeaderWidth(resultText) + 80.0F * Settings.scale,
               yOffset,
