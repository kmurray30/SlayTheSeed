...
public class DirectAndRawInputEnvironmentPlugin extends ControllerEnvironment {
   ...
   public Controller[] getControllers() {
      if (this.controllers == null) {
         ...
         for (int ix = 0; ix < dinputControllers.length; ix++) {
            if (dinputControllers[ix].getType() == Controller.Type.KEYBOARD) {
               if (!rawKeyboardFound) {
                  tempControllers.add(dinputControllers[ix]);
               }
            } else if (dinputControllers[ix].getType() == Controller.Type.MOUSE) {
               if (!rawMouseFound) {
                  tempControllers.add(dinputControllers[ix]);
               }
            } else {
