...
public class Model implements Disposable {
   ...
   protected void loadNodes(Iterable<ModelNode> modelNodes) {
      ...
      for (ObjectMap.Entry<NodePart, ArrayMap<String, Matrix4>> e : this.nodePartBones.entries()) {
   ...
   public Animation getAnimation(String id, boolean ignoreCase) {
      ...
      if (ignoreCase) {
         ...
         for (int ix = 0; ix < n; ix++) {
            Animation animation;
            if ((animation = this.animations.get(ix)).id.equals(id)) {
               return animation;
            }
         }
      }
   ...
   public Material getMaterial(String id, boolean ignoreCase) {
      ...
      if (ignoreCase) {
         ...
         for (int ix = 0; ix < n; ix++) {
            Material material;
            if ((material = this.materials.get(ix)).id.equals(id)) {
               return material;
            }
         }
      }
