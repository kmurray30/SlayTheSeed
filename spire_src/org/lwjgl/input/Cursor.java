...
public class Cursor {
   ...
   public Cursor(int width, int height, int xHotspot, int yHotspot, int numImages, IntBuffer images, IntBuffer delays) throws LWJGLException {
      synchronized (OpenGLPackageAccess.global_lock) {
         if ((getCapabilities() & 1) == 0) {
            ...
            } else if (yHotspot < height && yHotspot >= 0) {
               Sys.initialize();
   ...
   public static int getCapabilities() {
      synchronized (OpenGLPackageAccess.global_lock) {
         return Mouse.getImplementation() != null
            ? Mouse.getImplementation().getNativeCursorCapabilities()
            : OpenGLPackageAccess.createImplementation().getNativeCursorCapabilities();
      }
   }
      ...
      switch (LWJGLUtil.getPlatform()) {
         ...
         case 2:
            ...
            for (int i = 0; i < numImages; i++) {
               Object handlex = Mouse.getImplementation().createCursor(width, height, xHotspot, yHotspot, 1, images_copy, null);
               long delay = delays != null ? delays.get(i) : 0L;
               long timeout = System.currentTimeMillis();
               cursors[i] = new Cursor.CursorElement(handlex, delay, timeout);
               ((Buffer)images_copy).position(width * height * (i + 1));
            }
            break;
         case 3:
            cursors = new Cursor.CursorElement[numImages];
