...
public abstract class SimpleInfluencer extends Influencer {
   ...
   public void update() {
      ...
      for (int c = i + this.controller.particles.size * this.valueChannel.strideSize; i < c; l += this.lifeChannel.strideSize) {
         this.valueChannel.data[i] = this.interpolationChannel.data[a + 0]
            + this.interpolationChannel.data[a + 1] * this.value.getScale(this.lifeChannel.data[l]);
         i += this.valueChannel.strideSize;
