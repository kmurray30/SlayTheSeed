...
public class Encoder {
   ...
   int GetRepLen1Price(int state, int posState) {
      return com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(this._isRepG0[state])
         + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(this._isRep0Long[(state << 4) + posState]);
   }
         ...
         if (numAvailableBytes < 2) {
            ...
            if (this.repLens[repMaxIndex] >= this._numFastBytes) {
               ...
               if (lenMain < 2 && currentByte != matchByte && this.repLens[repMaxIndex] < 2) {
                  ...
                  this._optimum[1].Price = com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(this._isMatch[(this._state << 4) + posState])
                  ...
                  if (lenEnd < 2) {
                     ...
                     for (int var43 = 0; var43 < 4; var43++) {
                        int repLen = this.repLens[var43];
                        if (repLen >= 2) {
                           int price = repMatchPrice + this.GetPureRepPrice(var43, this._state, posState);

                           while (true) {
                              int curAndLenPrice = price + this._repMatchLenEncoder.GetPrice(repLen - 2, posState);
                              Encoder.Optimal optimum = this._optimum[repLen];
                              if (curAndLenPrice < optimum.Price) {
                                 optimum.Price = curAndLenPrice;
                                 optimum.PosPrev = 0;
                                 optimum.BackPrev = var43;
                     ...
                     if (len <= lenMain) {
                        ...
                        while (true) {
                           ...
                           int curAndLenPricex = normalMatchPrice + this.GetPosLenPrice(distance, len, posState);
                           Encoder.Optimal optimumx = this._optimum[len];
                           if (curAndLenPricex < optimumx.Price) {
                              optimumx.Price = curAndLenPricex;
                              optimumx.PosPrev = 0;
                              optimumx.BackPrev = distance + 4;
                              optimumx.Prev1IsChar = false;
                           }
                     ...
                     while (++cur != lenEnd) {
                        ...
                        int curAnd1Price = curPrice
                           + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(this._isMatch[(state << 4) + posState])
                           + this._literalEncoder
                              .GetSubCoder(position, this._matchFinder.GetIndexByte(-2))
                        ...
                        if (numAvailableBytesFull >= 2) {
                           ...
                           if (!nextIsChar && matchByte != currentByte) {
                              ...
                              if (lenTest2 >= 2) {
                                 ...
                                 int nextRepMatchPrice = curAnd1Price
                                    + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice1(this._isMatch[(state2 << 4) + posStateNext])
                                    + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice1(this._isRep[state2]);
                                 ...
                                 int curAndLenPricexx = nextRepMatchPrice + this.GetRepPrice(0, lenTest2, state2, posStateNext);
                                 Encoder.Optimal optimumxx = this._optimum[offset];
                                 if (curAndLenPricexx < optimumxx.Price) {
                                    optimumxx.Price = curAndLenPricexx;
                                    optimumxx.PosPrev = cur + 1;
                                    optimumxx.BackPrev = 0;
                                    optimumxx.Prev1IsChar = true;
                                    optimumxx.Prev2 = false;
                                 }
                              }
                           }
                           ...
                           for (int repIndex = 0; repIndex < 4; repIndex++) {
                              int lenTest = this._matchFinder.GetMatchLen(-1, this.reps[repIndex], numAvailableBytes);
                              if (lenTest >= 2) {
                                 int lenTestTemp = lenTest;

                                 while (true) {
                                    while (lenEnd >= cur + lenTest) {
                                       int curAndLenPricexx = repMatchPrice + this.GetRepPrice(repIndex, lenTest, state, posState);
                                       Encoder.Optimal optimumxx = this._optimum[cur + lenTest];
                                       if (curAndLenPricexx < optimumxx.Price) {
                                          optimumxx.Price = curAndLenPricexx;
                                          optimumxx.PosPrev = cur;
                                          optimumxx.BackPrev = repIndex;
                                          optimumxx.Prev1IsChar = false;
                                       }

                                       if (--lenTest < 2) {
                                          if (repIndex == 0) {
                                             startLen = lenTestTemp + 1;
                                          }
                                          ...
                                          curAndLenPricexx = Math.min(numAvailableBytesFull - 1 - lenTestTemp, this._numFastBytes);
                                          int lenTest2 = this._matchFinder.GetMatchLen(lenTestTemp, this.reps[repIndex], curAndLenPricexx);
                                          if (lenTest2 < 2) {
                                             continue label284;
                                          }

                                          int state2 = Base.StateUpdateRep(state);
                                          int posStateNext = position + lenTestTemp & this._posStateMask;
                                          int curAndLenCharPrice = repMatchPrice
                                             + this.GetRepPrice(repIndex, lenTestTemp, state, posState)
                                             + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(this._isMatch[(state2 << 4) + posStateNext])
                                             + this._literalEncoder
                                                .GetSubCoder(position + lenTestTemp, this._matchFinder.GetIndexByte(lenTestTemp - 1 - 1))
                                                .GetPrice(
                                                   true,
                                                   this._matchFinder.GetIndexByte(lenTestTemp - 1 - (this.reps[repIndex] + 1)),
                                                   this._matchFinder.GetIndexByte(lenTestTemp - 1)
                                                );
                                          state2 = Base.StateUpdateChar(state2);
                                          posStateNext = position + lenTestTemp + 1 & this._posStateMask;
                                          int nextMatchPrice = curAndLenCharPrice
                                             + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice1(this._isMatch[(state2 << 4) + posStateNext]);
                                          int nextRepMatchPrice = nextMatchPrice
                                             + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice1(this._isRep[state2]);
                                          int offset = lenTestTemp + 1 + lenTest2;

                                          while (lenEnd < cur + offset) {
                                             this._optimum[++lenEnd].Price = 268435455;
                                          }

                                          int curAndLenPricexxx = nextRepMatchPrice + this.GetRepPrice(0, lenTest2, state2, posStateNext);
                                          Encoder.Optimal optimumxxx = this._optimum[cur + offset];
                                          if (curAndLenPricexxx < optimumxxx.Price) {
                                             optimumxxx.Price = curAndLenPricexxx;
                                             optimumxxx.PosPrev = cur + lenTestTemp + 1;
                                             optimumxxx.BackPrev = 0;
                                             optimumxxx.Prev1IsChar = true;
                                             optimumxxx.Prev2 = true;
                                             optimumxxx.PosPrev2 = cur;
                                             optimumxxx.BackPrev2 = repIndex;
                                          }
                                          continue label284;
                                       }
                                    }
                           ...
                           if (newLen > numAvailableBytes) {
                              ...
                              while (newLen > this._matchDistances[var40]) {
                                 var40 += 2;
                              }

                              this._matchDistances[var40] = newLen;
                              numDistancePairs = var40 + 2;
                           }
                           ...
                           if (newLen >= startLen) {
                              ...
                              while (true) {
                                 ...
                                 int curAndLenPricexxx = normalMatchPrice + this.GetPosLenPrice(curBack, lenTest, posState);
                                 Encoder.Optimal optimumxxx = this._optimum[cur + lenTest];
                                 if (curAndLenPricexxx < optimumxxx.Price) {
                                    optimumxxx.Price = curAndLenPricexxx;
                                    optimumxxx.PosPrev = cur;
                                    optimumxxx.BackPrev = curBack + 4;
                                    optimumxxx.Prev1IsChar = false;
                                 }

                                 if (lenTest == this._matchDistances[offs]) {
                                    if (lenTest < numAvailableBytesFull) {
                                       int t = Math.min(numAvailableBytesFull - 1 - lenTest, this._numFastBytes);
                                       int lenTest2x = this._matchFinder.GetMatchLen(lenTest, curBack, t);
                                       if (lenTest2x >= 2) {
                                          int state2 = Base.StateUpdateMatch(state);
                                          int posStateNext = position + lenTest & this._posStateMask;
                                          int curAndLenCharPrice = curAndLenPricexxx
                                             + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice0(this._isMatch[(state2 << 4) + posStateNext])
                                             + this._literalEncoder
                                                .GetSubCoder(position + lenTest, this._matchFinder.GetIndexByte(lenTest - 1 - 1))
                                                .GetPrice(
                                                   true,
                                          ...
                                          int nextRepMatchPrice = nextMatchPrice
                                             + com.badlogic.gdx.utils.compression.rangecoder.Encoder.GetPrice1(this._isRep[state2]);
                                          int offset = lenTest + 1 + lenTest2x;

                                          while (lenEnd < cur + offset) {
                                             this._optimum[++lenEnd].Price = 268435455;
                                          }

                                          curAndLenPricexxx = nextRepMatchPrice + this.GetRepPrice(0, lenTest2x, state2, posStateNext);
                                          optimumxxx = this._optimum[cur + offset];
                                          if (curAndLenPricexxx < optimumxxx.Price) {
                                             optimumxxx.Price = curAndLenPricexxx;
                                             optimumxxx.PosPrev = cur + lenTest + 1;
                                             optimumxxx.BackPrev = 0;
                                             optimumxxx.Prev1IsChar = true;
                                             optimumxxx.Prev2 = true;
                                             optimumxxx.PosPrev2 = cur;
                                             optimumxxx.BackPrev2 = curBack + 4;
                                          }
                                       }
                                    }
   ...
   void FillDistancesPrices() {
      ...
      for (int lenToPosState = 0; lenToPosState < 4; lenToPosState++) {
         ...
         for (int var9 = 14; var9 < this._distTableSize; var9++) {
            this._posSlotPrices[st + var9] = this._posSlotPrices[st + var9] + ((var9 >> 1) - 1 - 4 << 6);
         }
