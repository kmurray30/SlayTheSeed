...
public class Hexaghost extends AbstractMonster {
   ...
   public void takeTurn() {
      switch (this.nextMove) {
            ...
            for (int i = 0; i < 6; i++) {
               AbstractDungeon.actionManager
                  .addToBottom(
                     new VFXAction(
                        this,
                        new GhostIgniteEffect(
                           AbstractDungeon.player.hb.cX + MathUtils.random(-120.0F, 120.0F) * Settings.scale,
                           AbstractDungeon.player.hb.cY + MathUtils.random(-120.0F, 120.0F) * Settings.scale
                        ),
                        0.05F
                     )
                  );
               ...
               AbstractDungeon.actionManager
                  .addToBottom(new DamageAction(AbstractDungeon.player, this.damage.get(2), AbstractGameAction.AttackEffect.BLUNT_HEAVY, true));
            }
         ...
         case 4:
            AbstractDungeon.actionManager
               .addToBottom(new VFXAction(new FireballEffect(this.hb.cX, this.hb.cY, AbstractDungeon.player.hb.cX, AbstractDungeon.player.hb.cY), 0.5F));
         ...
         case 5:
            ...
            this.applyPowers();
            this.setMove((byte)1, AbstractMonster.Intent.ATTACK, this.damage.get(2).base, 6, true);
            break;
         case 6:
            AbstractDungeon.actionManager.addToBottom(new VFXAction(this, new ScreenOnFireEffect(), 1.0F));

            for (int i = 0; i < this.infernoHits; i++) {
               AbstractDungeon.actionManager.addToBottom(new DamageAction(AbstractDungeon.player, this.damage.get(3), AbstractGameAction.AttackEffect.FIRE));
            }
   ...
   protected void getMove(int num) {
      if (!this.activated) {
         ...
         switch (this.orbActiveCount) {
            case 0:
               this.setMove(SEAR_NAME, (byte)4, AbstractMonster.Intent.ATTACK_DEBUFF, this.damage.get(1).base);
               break;
            case 1:
               this.setMove((byte)2, AbstractMonster.Intent.ATTACK, this.damage.get(0).base, this.fireTackleCount, true);
               break;
            case 2:
               this.setMove(SEAR_NAME, (byte)4, AbstractMonster.Intent.ATTACK_DEBUFF, this.damage.get(1).base);
               break;
            ...
            case 5:
               ...
               break;
            case 6:
               this.setMove(BURN_NAME, (byte)6, AbstractMonster.Intent.ATTACK_DEBUFF, this.damage.get(3).base, this.infernoHits, true);
         }
      }
   }
   ...
   public void changeState(String stateName) {
      switch (stateName) {
         case "Activate":
            ...
            for (HexaghostOrb orbx : this.orbs) {
               orbx.activate(this.drawX + this.animX, this.drawY + this.animY);
            }
