...
public final class LayerIIIDecoder implements FrameDecoder {
   ...
   public static final int[][][] nr_of_sfb_block = new int[][][]{
      ...
      if (bytes_to_discard >= 0) {
         ...
         for (int gr = 0; gr < this.max_gr; gr++) {
            ...
            for (int var11 = this.first_channel; var11 <= this.last_channel; var11++) {
               this.reorder(this.lr[var11], var11, gr);
               this.antialias(var11, gr);
               this.hybrid(var11, gr);
               ...
               if (var11 != 0 && this.which_channels != 2) {
                  for (int ss = 0; ss < 18; ss++) {
                     ...
                     for (int var16 = 0; var16 < 576; var16 += 18) {
                        this.samples2[sb] = this.out_1d[var16 + ss];
                        sb++;
                     }
                  ...
                  for (int ss = 0; ss < 18; ss++) {
                     ...
                     for (int var15 = 0; var15 < 576; var15 += 18) {
                        this.samples1[sb] = this.out_1d[var15 + ss];
                        sb++;
                     }
      ...
      if (this.header.version() == 1) {
         ...
         for (int gr = 0; gr < 2; gr++) {
            for (int var3 = 0; var3 < this.channels; var3++) {
               this.si.ch[var3].gr[gr].part2_3_length = this.stream.get_bits(12);
               this.si.ch[var3].gr[gr].big_values = this.stream.get_bits(9);
               this.si.ch[var3].gr[gr].global_gain = this.stream.get_bits(8);
               this.si.ch[var3].gr[gr].scalefac_compress = this.stream.get_bits(4);
               this.si.ch[var3].gr[gr].window_switching_flag = this.stream.get_bits(1);
               if (this.si.ch[var3].gr[gr].window_switching_flag != 0) {
                  this.si.ch[var3].gr[gr].block_type = this.stream.get_bits(2);
                  this.si.ch[var3].gr[gr].mixed_block_flag = this.stream.get_bits(1);
                  this.si.ch[var3].gr[gr].table_select[0] = this.stream.get_bits(5);
                  this.si.ch[var3].gr[gr].table_select[1] = this.stream.get_bits(5);
                  this.si.ch[var3].gr[gr].subblock_gain[0] = this.stream.get_bits(3);
                  this.si.ch[var3].gr[gr].subblock_gain[1] = this.stream.get_bits(3);
                  this.si.ch[var3].gr[gr].subblock_gain[2] = this.stream.get_bits(3);
                  if (this.si.ch[var3].gr[gr].block_type == 0) {
                     return false;
                  }

                  if (this.si.ch[var3].gr[gr].block_type == 2 && this.si.ch[var3].gr[gr].mixed_block_flag == 0) {
                     this.si.ch[var3].gr[gr].region0_count = 8;
                  } else {
                     this.si.ch[var3].gr[gr].region0_count = 7;
                  }

                  this.si.ch[var3].gr[gr].region1_count = 20 - this.si.ch[var3].gr[gr].region0_count;
               } else {
                  this.si.ch[var3].gr[gr].table_select[0] = this.stream.get_bits(5);
                  this.si.ch[var3].gr[gr].table_select[1] = this.stream.get_bits(5);
                  this.si.ch[var3].gr[gr].table_select[2] = this.stream.get_bits(5);
                  this.si.ch[var3].gr[gr].region0_count = this.stream.get_bits(4);
                  this.si.ch[var3].gr[gr].region1_count = this.stream.get_bits(3);
                  this.si.ch[var3].gr[gr].block_type = 0;
               }

               this.si.ch[var3].gr[gr].preflag = this.stream.get_bits(1);
               this.si.ch[var3].gr[gr].scalefac_scale = this.stream.get_bits(1);
               this.si.ch[var3].gr[gr].count1table_select = this.stream.get_bits(1);
            }
         }
      } else {
         ...
         if (gr_info.mixed_block_flag != 0) {
            ...
            for (int var9 = 3; var9 < 6; var9++) {
               for (int window = 0; window < 3; window++) {
                  this.scalefac[ch].s[window][var9] = this.br.hgetbits(slen[0][gr_info.scalefac_compress]);
               }
            }

            for (int var10 = 6; var10 < 12; var10++) {
               for (int window = 0; window < 3; window++) {
                  this.scalefac[ch].s[window][var10] = this.br.hgetbits(slen[1][gr_info.scalefac_compress]);
               }
            }
            ...
            for (int window = 0; window < 3; window++) {
               this.scalefac[ch].s[window][var11] = 0;
            }
         } else {
   ...
   private void get_LSF_scale_factors(int ch, int gr) {
      ...
      if (gr_info.window_switching_flag == 0 || gr_info.block_type != 2) {
         ...
         for (int var7 = 3; var7 < 12; var7++) {
            for (int window = 0; window < 3; window++) {
               this.scalefac[ch].s[window][var7] = this.scalefac_buffer[m];
               m++;
            }
         }
   ...
   private void dequantize_sample(float[][] xr, int ch, int gr) {
      ...
      float g_gain = (float)Math.pow(2.0, 0.25 * (gr_info.global_gain - 210.0));
      ...
      for (int var23 = 0; var23 < this.nonzero[ch]; var23++) {
         int reste = var23 % 18;
         int quotien = (var23 - reste) / 18;
         ...
         if (gr_info.window_switching_flag == 0
            || (gr_info.block_type != 2 || gr_info.mixed_block_flag != 0) && (gr_info.block_type != 2 || gr_info.mixed_block_flag == 0 || var23 < 36)) {
      ...
      for (int var24 = this.nonzero[ch]; var24 < 576; var24++) {
         int restex = var24 % 18;
         int quotienx = (var24 - restex) / 18;
         if (restex < 0) {
            restex = 0;
         }
      ...
      if (gr_info.window_switching_flag != 0 && gr_info.block_type == 2) {
         ...
         if (gr_info.mixed_block_flag != 0) {
            for (int var18 = 0; var18 < 36; var18++) {
               int reste = var18 % 18;
               int quotien = (var18 - reste) / 18;
               this.out_1d[var18] = xr_1d[quotien][reste];
            }
            ...
            for (int var19 = 0; var19 < 576; var19++) {
               int j = reorder_table[this.sfreq][var19];
               int reste = j % 18;
               int quotien = (j - reste) / 18;
               this.out_1d[var19] = xr_1d[quotien][reste];
            }
         }
      } else {
         for (int index = 0; index < 576; index++) {
      ...
      if (this.channels == 1) {
         ...
         if (i_stereo) {
            if (gr_info.window_switching_flag != 0 && gr_info.block_type == 2) {
               if (gr_info.mixed_block_flag != 0) {
                  ...
                  for (int j = 0; j < 3; j++) {
                     ...
                     for (int sfb = 12; sfb >= 3; sfb--) {
                        int var48 = this.sfBandIndex[this.sfreq].s[sfb];
                        int lines = this.sfBandIndex[this.sfreq].s[sfb + 1] - var48;

                        for (int var49 = (var48 << 2) - var48 + (j + 1) * lines - 1; lines > 0; var49--) {
                     ...
                     while (var38 < 12) {
                        int temp = this.sfBandIndex[this.sfreq].s[var38];
                        int sb = this.sfBandIndex[this.sfreq].s[var38 + 1] - temp;

                        for (int var50 = (temp << 2) - temp + j * sb; sb > 0; sb--) {
                           this.is_pos[var50] = this.scalefac[1].s[j][var38];
                           if (this.is_pos[var50] != 7) {
                              if (lsf) {
                                 this.i_stereo_k_values(this.is_pos[var50], io_type, var50);
                              } else {
                     ...
                     int sb = this.sfBandIndex[this.sfreq].s[11] - var38;
                     var38 = (var38 << 2) - var38 + j * sb;
                     int temp = this.sfBandIndex[this.sfreq].s[11];
                     sb = this.sfBandIndex[this.sfreq].s[12] - temp;

                     for (int var51 = (temp << 2) - temp + j * sb; sb > 0; sb--) {
                  ...
                  if (max_sfb <= 3) {
                     ...
                     while (var52 >= 0) {
                        if (this.ro[1][var52][ss] != 0.0F) {
                           sb = (var52 << 4) + (var52 << 1) + ss;
                           var52 = -1;
                        } else if (--ss < 0) {
                           var52--;
                           ss = 17;
                     ...
                     while (this.sfBandIndex[this.sfreq].l[var52] <= sb) {
                        var52++;
                     }

                     int sfb = var52;

                     for (int var54 = this.sfBandIndex[this.sfreq].l[var52]; sfb < 8; sfb++) {
                        for (int var28 = this.sfBandIndex[this.sfreq].l[sfb + 1] - this.sfBandIndex[this.sfreq].l[sfb]; var28 > 0; var28--) {
                           this.is_pos[var54] = this.scalefac[1].l[sfb];
                           if (this.is_pos[var54] != 7) {
                              if (lsf) {
                                 this.i_stereo_k_values(this.is_pos[var54], io_type, var54);
                              } else {
                  ...
                  for (int j = 0; j < 3; j++) {
                     ...
                     for (int sfb = 12; sfb >= 0; sfb--) {
                        int temp = this.sfBandIndex[this.sfreq].s[sfb];
                        int lines = this.sfBandIndex[this.sfreq].s[sfb + 1] - temp;

                        for (int var45 = (temp << 2) - temp + (j + 1) * lines - 1; lines > 0; var45--) {
                     ...
                     for (int var35 = sfbcnt + 1; var35 < 12; var35++) {
                        int temp = this.sfBandIndex[this.sfreq].s[var35];
                        int sb = this.sfBandIndex[this.sfreq].s[var35 + 1] - temp;

                        for (int var46 = (temp << 2) - temp + j * sb; sb > 0; sb--) {
                           this.is_pos[var46] = this.scalefac[1].s[j][var35];
                           if (this.is_pos[var46] != 7) {
                              if (lsf) {
                                 this.i_stereo_k_values(this.is_pos[var46], io_type, var46);
                              } else {
                     ...
                     int sb = temp2 - temp;
                     int var36 = (temp << 2) - temp + j * sb;
                     sb = this.sfBandIndex[this.sfreq].s[12] - temp2;

                     for (int var47 = (temp2 << 2) - temp2 + j * sb; sb > 0; sb--) {
               ...
               while (var42 >= 0) {
                  if (this.ro[1][var42][ss] != 0.0F) {
                     sb = (var42 << 4) + (var42 << 1) + ss;
                     var42 = -1;
                  } else if (--ss < 0) {
                     var42--;
                     ss = 17;
               ...
               while (this.sfBandIndex[this.sfreq].l[var42] <= sb) {
                  var42++;
               }

               int sfb = var42;

               for (var42 = this.sfBandIndex[this.sfreq].l[var42]; sfb < 21; sfb++) {
                  for (int var19 = this.sfBandIndex[this.sfreq].l[sfb + 1] - this.sfBandIndex[this.sfreq].l[sfb]; var19 > 0; var19--) {
                     this.is_pos[var42] = this.scalefac[1].l[sfb];
                     if (this.is_pos[var42] != 7) {
                        if (lsf) {
                           this.i_stereo_k_values(this.is_pos[var42], io_type, var42);
                        } else {
               ...
               for (int var20 = 576 - this.sfBandIndex[this.sfreq].l[21]; var20 > 0 && var42 < 576; var20--) {
                  this.is_pos[var42] = this.is_pos[sfb];
                  if (lsf) {
                     this.k[0][var42] = this.k[0][sfb];
                     this.k[1][var42] = this.k[1][sfb];
                  } else {
                     this.is_ratio[var42] = this.is_ratio[sfb];
                  }
         ...
         for (int sb = 0; sb < 32; sb++) {
            for (int ss = 0; ss < 18; ss++) {
               if (this.is_pos[var55] == 7) {
                  ...
                  if (lsf) {
                     this.lr[0][sb][ss] = this.ro[0][sb][ss] * this.k[0][var55];
                     this.lr[1][sb][ss] = this.ro[0][sb][ss] * this.k[1][var55];
                  } else {
                     this.lr[1][sb][ss] = this.ro[0][sb][ss] / (1.0F + this.is_ratio[var55]);
                     this.lr[0][sb][ss] = this.lr[1][sb][ss] * this.is_ratio[var55];
                  }
               }
   ...
   public void inv_mdct(float[] in, float[] out, int block_type) {
      ...
      if (block_type == 2) {
         ...
         for (int i = 0; i < 3; i++) {
            ...
            tmpf_4 *= 0.70710677F;
            tmpf_5 *= 0.5176381F;
            float var51 = tmpf_0 + tmpf_5;
            tmpf_5 = tmpf_0 - tmpf_5;
            float var57 = tmpf_1 + tmpf_4;
            tmpf_4 = tmpf_1 - tmpf_4;
            float var62 = tmpf_2 + tmpf_3;
            tmpf_3 = tmpf_2 - tmpf_3;
            tmpf_0 = var51 * 0.5043145F;
            tmpf_1 = var57 * 0.5411961F;
            tmpf_2 = var62 * 0.6302362F;
            tmpf_3 *= 0.8213398F;
            tmpf_4 *= 1.306563F;
            tmpf_5 *= 3.830649F;
