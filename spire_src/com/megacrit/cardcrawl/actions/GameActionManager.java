...
public class GameActionManager {
   ...
   public GameActionManager.Phase phase = GameActionManager.Phase.WAITING_ON_USER;
   ...
   private void getNextAction() {
      if (!this.actions.isEmpty()) {
         ...
         if (this.cardQueue.get(0).card == null
            || !c.canUse(AbstractDungeon.player, this.cardQueue.get(0).monster) && !this.cardQueue.get(0).card.dontTriggerOnUseCard) {
            ...
            if (this.cardQueue.get(0).card != null) {
               if (this.cardQueue.get(0).card.target == AbstractCard.CardTarget.ENEMY
                  && (this.cardQueue.get(0).monster == null || this.cardQueue.get(0).monster.isDeadOrEscaped())) {
                  Iterator<AbstractCard> ix = AbstractDungeon.player.limbo.group.iterator();

                  while (ix.hasNext()) {
                     AbstractCard e = ix.next();
                     if (e == this.cardQueue.get(0).card) {
         ...
         if (!m.isDeadOrEscaped() || m.halfDead) {
            ...
            if (!TipTracker.tips.get("INTENT_TIP")
   ...
   public void cleanCardQueue() {
      ...
      for (AbstractCard e : AbstractDungeon.player.limbo.group) {
         e.fadingOut = true;
      }
   }
