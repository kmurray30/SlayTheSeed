...
public class UnicodeFont {
   ...
   UnicodeFont.RenderType renderType;
   ...
   public boolean loadGlyphs(int maxGlyphsToLoad) {
      if (this.queuedGlyphs.isEmpty()) {
         ...
         while (!this.queuedGlyphs.isEmpty()) {
            GlyphPage glyphPagex = new GlyphPage(this, this.glyphPageWidth, this.glyphPageHeight);
            this.glyphPages.add(glyphPagex);
            maxGlyphsToLoad -= glyphPagex.loadGlyphs(this.queuedGlyphs, maxGlyphsToLoad);
   ...
   public void drawString(float x, float y, String text, Color color, int startIndex, int endIndex) {
      if (text == null) {
         ...
         if (color == null) {
            ...
            if (this.renderType == UnicodeFont.RenderType.FreeType && this.bitmapFont != null) {
   ...
   private void drawBitmap(String text, int startIndex, int endIndex) {
      ...
      for (int n = regions.size; i < n; i++) {
         ...
         for (int nn = vertices.length; ii < nn; ii += 20) {
      ...
      for (int n = vector.getNumGlyphs(); glyphIndex < n; glyphIndex++) {
         ...
         if (charIndex >= startIndex) {
            ...
            if (codePoint == 10) {
               ...
               lines++;
               totalHeight = 0;
            } else if (this.renderType == UnicodeFont.RenderType.Native) {
   ...
   private Rectangle getGlyphBounds(GlyphVector vector, int index, int codePoint) {
      ...
      if (this.renderType == UnicodeFont.RenderType.Native) {
         if (bounds.width != 0 && bounds.height != 0) {
            bounds = this.metrics.getStringBounds("" + (char)codePoint, GlyphPage.scratchGraphics).getBounds();
         } else {
   ...
   public int getYOffset(String text) {
      if (text == null) {
         throw new IllegalArgumentException("text cannot be null.");
      } else if (this.renderType == UnicodeFont.RenderType.FreeType && this.bitmapFont != null) {
         ...
         char[] chars = text.toCharArray();
         GlyphVector vector = this.font.layoutGlyphVector(GlyphPage.renderContext, chars, 0, chars.length, 0);
         return this.ascent + vector.getPixelBounds(null, 0.0F, 0.0F).y;
      }
   }
   ...
   public UnicodeFont.RenderType getRenderType() {
      return this.renderType;
   }

   public void setRenderType(UnicodeFont.RenderType renderType) {
      this.renderType = renderType;
      if (renderType != UnicodeFont.RenderType.FreeType) {
         if (this.bitmapFont != null) {
            this.bitmapFont.dispose();
            this.generator.dispose();
         }
   ...
   private static Font createFont(String ttfFileRef) {
      try {
         ...
         throw new GdxRuntimeException("Invalid font: " + ttfFileRef, var2);
      } catch (IOException var3) {
         throw new GdxRuntimeException("Error reading font: " + ttfFileRef, var3);
      }
   }
