         ...
         if (fPrev.next == fHead) {
            if ($assertionsDisabled || f.prev == fPrev && f.anEdge == null && f.data == null) {
               ...
               while (true) {
                  ...
                  GLUhalfEdge ex = v.anEdge;

                  while ($assertionsDisabled || ex.Sym != ex) {
                     assert ex.Sym.Sym == ex;

                     assert ex.Lnext.Onext.Sym == ex;

                     assert ex.Onext.Sym.Lnext == ex;
