...
public class ShopScreen {
   ...
   public void applyUpgrades(AbstractCard.CardType type) {
      ...
      for (AbstractCard cx : this.colorlessCards) {
         if (cx.type == type) {
            cx.upgrade();
         }
      }
   ...
   private void setStartingCardPositions() {
      ...
      for (int i = 0; i < this.colorlessCards.size(); i++) {
         ...
         this.colorlessCards.get(i).current_x = DRAW_START_X + AbstractCard.IMG_WIDTH_S / 2.0F + padX * i;
         this.colorlessCards.get(i).target_x = DRAW_START_X + AbstractCard.IMG_WIDTH_S / 2.0F + padX * i;
   ...
   public void update() {
      ...
      if (this.handTimer != 0.0F) {
         this.handTimer = this.handTimer - Gdx.graphics.getDeltaTime();
         if (this.handTimer < 0.0F) {
            this.handTimer = 0.0F;
         }
      }
      ...
      for (AbstractCard cx : this.colorlessCards) {
         if (cx.hb.hovered) {
            hoveredCard = cx;
            this.somethingHovered = true;
            this.moveHand(cx.current_x - AbstractCard.IMG_WIDTH / 2.0F, cx.current_y);
            break;
         }
      }

      if (!this.somethingHovered) {
         this.notHoveredTimer = this.notHoveredTimer + Gdx.graphics.getDeltaTime();
   ...
   private void updateControllerInput() {
      if (Settings.isControllerMode
         ...
         ShopScreen.StoreSelectionType type = null;
         int index = 0;

         for (AbstractCard c : this.coloredCards) {
            if (c.hb.hovered) {
               type = ShopScreen.StoreSelectionType.COLOR_CARD;
               break;
            }
         ...
         if (type == null) {
            ...
            for (StoreRelic r : this.relics) {
               if (r.relic.hb.hovered) {
                  type = ShopScreen.StoreSelectionType.RELIC;
                  break;
               }
         ...
         if (type == null) {
            ...
            for (AbstractCard c : this.colorlessCards) {
               if (c.hb.hovered) {
                  type = ShopScreen.StoreSelectionType.COLORLESS_CARD;
                  break;
               }
         ...
         if (type == null) {
            ...
            for (StorePotion p : this.potions) {
               if (p.potion.hb.hovered) {
                  type = ShopScreen.StoreSelectionType.POTION;
                  break;
               }
         ...
         if (type == null && this.purgeHovered) {
            type = ShopScreen.StoreSelectionType.PURGE;
         }
            ...
            switch (type) {
               case COLOR_CARD:
                  if (CInputActionSet.left.isJustPressed() || CInputActionSet.altLeft.isJustPressed()) {
                     ...
                     Gdx.input.setCursorPosition((int)this.coloredCards.get(index).hb.cX, Settings.HEIGHT - (int)this.coloredCards.get(index).hb.cY);
               ...
               case RELIC:
                     ...
                     if (--index < 0 && !this.colorlessCards.isEmpty()) {
                        Gdx.input
                           .setCursorPosition(
                              ...
                              Settings.HEIGHT - (int)this.colorlessCards.get(this.colorlessCards.size() - 1).hb.cY
                           );
                     } else {
                        if (index < 0) {
                           index = 0;
                  ...
                  } else if (++index > this.relics.size() - 1 && this.purgeAvailable) {
                     Gdx.input.setCursorPosition((int)this.purgeCardX, Settings.HEIGHT - (int)this.purgeCardY);
               ...
               case PURGE:
                  if (!CInputActionSet.left.isJustPressed() && !CInputActionSet.altLeft.isJustPressed()) {
                     if ((CInputActionSet.up.isJustPressed() || CInputActionSet.altUp.isJustPressed()) && !this.coloredCards.isEmpty()) {
                        Gdx.input
                           .setCursorPosition(
                              (int)this.coloredCards.get(this.coloredCards.size() - 1).hb.cX,
                              Settings.HEIGHT - (int)this.coloredCards.get(this.coloredCards.size() - 1).hb.cY
                           );
                     }
                  } else if (!this.relics.isEmpty()) {
                     Gdx.input
                        .setCursorPosition(
                           (int)this.relics.get(this.relics.size() - 1).relic.hb.cX, Settings.HEIGHT - (int)this.relics.get(this.relics.size() - 1).relic.hb.cY
                        );
                  } else if (!this.potions.isEmpty()) {
                     Gdx.input
                        .setCursorPosition(
                           (int)this.potions.get(this.potions.size() - 1).potion.hb.cX,
                           Settings.HEIGHT - (int)this.potions.get(this.potions.size() - 1).potion.hb.cY
                        );
                  } else if (this.colorlessCards.isEmpty()) {
                     Gdx.input
                        .setCursorPosition(
   ...
   private void updatePurgeCard() {
      ...
      if (this.purgeAvailable) {
         ...
         if (InputHelper.mX > this.purgeCardX - CARD_W
            && InputHelper.mX < this.purgeCardX + CARD_W
            && InputHelper.mY > this.purgeCardY - CARD_H
            && InputHelper.mY < this.purgeCardY + CARD_H) {
   ...
   private void purchasePurge() {
      ...
      if (AbstractDungeon.player.gold >= actualPurgeCost) {
         ...
         AbstractDungeon.gridSelectScreen
            .open(CardGroup.getGroupWithoutBottledCards(AbstractDungeon.player.masterDeck.getPurgeableCards()), 1, NAMES[13], false, false, true, true);
      } else {
         this.playCantBuySfx();
         this.createSpeech(getCantBuyMsg());
      }
   ...
   private void updateSpeech() {
      ...
      this.speechTimer = this.speechTimer - Gdx.graphics.getDeltaTime();
   ...
   private void updateRug() {
      if (this.rugY != 0.0F) {
         this.rugY = MathUtils.lerp(this.rugY, Settings.HEIGHT / 2.0F - 540.0F * Settings.yScale, Gdx.graphics.getDeltaTime() * 5.0F);
   ...
   private void renderPurge(SpriteBatch sb) {
      ...
      sb.draw(
         tmpImg,
         this.purgeCardX + 18.0F * Settings.scale + tmpImg.offsetX - tmpImg.originalWidth / 2.0F,
         this.purgeCardY - 14.0F * Settings.scale + tmpImg.offsetY - tmpImg.originalHeight / 2.0F,
         tmpImg.originalWidth / 2.0F - tmpImg.offsetX,
         tmpImg.originalHeight / 2.0F - tmpImg.offsetY,
         tmpImg.packedWidth,
         tmpImg.packedHeight,
         this.purgeCardScale,
         this.purgeCardScale,
      ...
      if (this.purgeAvailable) {
         sb.draw(
            removeServiceImg,
            this.purgeCardX - 256.0F,
            this.purgeCardY - 256.0F,
            256.0F,
            256.0F,
            512.0F,
            512.0F,
         ...
         sb.draw(
            ImageMaster.UI_GOLD,
            this.purgeCardX + GOLD_IMG_OFFSET_X,
            this.purgeCardY + GOLD_IMG_OFFSET_Y - (this.purgeCardScale / Settings.scale - 0.75F) * 200.0F * Settings.scale,
            GOLD_IMG_WIDTH,
            GOLD_IMG_WIDTH
         );
         ...
         FontHelper.renderFontLeftTopAligned(
            ...
            this.purgeCardX + PRICE_TEXT_OFFSET_X,
            this.purgeCardY + PRICE_TEXT_OFFSET_Y - (this.purgeCardScale / Settings.scale - 0.75F) * 200.0F * Settings.scale,
            color
         );
      } else {
         sb.draw(
