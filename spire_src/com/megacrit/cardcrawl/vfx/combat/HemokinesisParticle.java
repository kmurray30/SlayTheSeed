...
public class HemokinesisParticle extends AbstractGameEffect {
   ...
   private void updateMovement() {
      ...
      this.rotationRate = this.rotationRate + Gdx.graphics.getDeltaTime() * 2000.0F;
      this.scale = this.scale + Gdx.graphics.getDeltaTime() * 1.0F * Settings.scale;
      ...
      if (this.controlPoints.size() > 3) {
         ...
         for (int ix = 0; ix < 60; ix++) {
            this.points[ix] = new Vector2();
            this.crs.valueAt(this.points[ix], ix / 59.0F);
         }
      }
   ...
   public void render(SpriteBatch sb) {
      if (!this.isDone) {
         ...
         for (int ix = this.points.length - 1; ix > 0; ix--) {
            if (this.points[ix] != null) {
               sb.draw(
                  this.img,
                  this.points[ix].x - this.img.packedWidth / 2,
                  this.points[ix].y - this.img.packedHeight / 2,
                  this.img.packedWidth / 2.0F,
                  this.img.packedHeight / 2.0F,
                  this.img.packedWidth,
                  this.img.packedHeight,
