...
class NewSlider extends JPanel {
   ...
   protected void paintComponent(Graphics graphics) {
      ...
      int knobX = (int)((width - this.border * 2 - 10) * (this.value - this.sliderMin) / (this.sliderMax - this.sliderMin)) + this.border;
      g.setColor(this.knobColor);
      g.fillRect(Math.max(this.border, Math.min(maxKnobX, knobX)), 0, 10, height);
      float displayValue = (int)(this.value * 10.0F) / 10.0F;
   ...
   public void setValue(float value) {
      this.value = (int)(Math.max(this.min, Math.min(this.max, value)) / this.stepSize) * this.stepSize;
      this.repaint();
