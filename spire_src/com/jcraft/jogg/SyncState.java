   ...
   public int pageseek(Page og) {
      ...
      if (this.headerbytes == 0) {
         ...
         for (int i = 0; i < (this.data[page + 26] & 255); i++) {
            this.bodybytes = this.bodybytes + (this.data[page + 27 + i] & 255);
         }
      ...
      if (this.bodybytes + this.headerbytes > bytes) {
         ...
         synchronized (this.chksum) {
            ...
            log.checksum();
            if (this.chksum[0] != this.data[page + 22]
               || this.chksum[1] != this.data[page + 23]
               || this.chksum[2] != this.data[page + 24]
               || this.chksum[3] != this.data[page + 25]) {
               System.arraycopy(this.chksum, 0, this.data, page + 22, 4);
               this.headerbytes = 0;
               this.bodybytes = 0;
               int next = 0;

               for (int iix = 0; iix < bytes - 1; iix++) {
                  if (this.data[page + 1 + iix] == 79) {
                     next = page + 1 + iix;
                     break;
                  }
               }
         ...
         this.unsynced = 0;
         this.returned = this.returned + (bytes = this.headerbytes + this.bodybytes);
         this.headerbytes = 0;
         this.bodybytes = 0;
         return bytes;
      }
