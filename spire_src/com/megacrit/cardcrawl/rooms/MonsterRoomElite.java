      ...
      if (Settings.isFinalActAvailable && AbstractDungeon.getCurrMapNode().hasEmeraldKey) {
         switch (AbstractDungeon.mapRng.random(0, 3)) {
            case 0:
               for (AbstractMonster m : this.monsters.monsters) {
                  AbstractDungeon.actionManager
                     .addToBottom(new ApplyPowerAction(m, m, new StrengthPower(m, AbstractDungeon.actNum + 1), AbstractDungeon.actNum + 1));
               }
               break;
            case 1:
               for (AbstractMonster m : this.monsters.monsters) {
            ...
            case 2:
               for (AbstractMonster m : this.monsters.monsters) {
                  AbstractDungeon.actionManager
                     .addToBottom(new ApplyPowerAction(m, m, new MetallicizePower(m, AbstractDungeon.actNum * 2 + 2), AbstractDungeon.actNum * 2 + 2));
               }
               break;
            case 3:
               for (AbstractMonster m : this.monsters.monsters) {
                  AbstractDungeon.actionManager
                     .addToBottom(new ApplyPowerAction(m, m, new RegenerateMonsterPower(m, 1 + AbstractDungeon.actNum * 2), 1 + AbstractDungeon.actNum * 2));
               }
         }
      }
   }
