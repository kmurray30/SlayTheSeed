...
public final class Header {
   ...
   private static final String[][][] bitrate_str = new String[][][]{
      {
            {
                  "free format",
                  "32 kbit/s",
   ...
   void parseVBR(byte[] firstframe) throws BitstreamException {
      ...
      try {
         System.arraycopy(firstframe, var9, tmp, 0, 4);
         if (xing.equals(new String(tmp))) {
            ...
            System.arraycopy(firstframe, var9 + length, flags, 0, flags.length);
            length += flags.length;
            if ((flags[3] & 1) != 0) {
               System.arraycopy(firstframe, var9 + length, tmp, 0, tmp.length);
               this.h_vbr_frames = tmp[0] << 24 & 0xFF000000 | tmp[1] << 16 & 0xFF0000 | tmp[2] << 8 & 0xFF00 | tmp[3] & 255;
               length += 4;
            }

            if ((flags[3] & 2) != 0) {
               System.arraycopy(firstframe, var9 + length, tmp, 0, tmp.length);
               this.h_vbr_bytes = tmp[0] << 24 & 0xFF000000 | tmp[1] << 16 & 0xFF0000 | tmp[2] << 8 & 0xFF00 | tmp[3] & 255;
               length += 4;
            }

            if ((flags[3] & 4) != 0) {
               System.arraycopy(firstframe, var9 + length, this.h_vbr_toc, 0, this.h_vbr_toc.length);
               length += this.h_vbr_toc.length;
            }

            if ((flags[3] & 8) != 0) {
               System.arraycopy(firstframe, var9 + length, tmp, 0, tmp.length);
               this.h_vbr_scale = tmp[0] << 24 & 0xFF000000 | tmp[1] << 16 & 0xFF0000 | tmp[2] << 8 & 0xFF00 | tmp[3] & 255;
               length += 4;
            }
         }
      ...
      try {
         ...
         if (vbri.equals(new String(tmp))) {
            ...
            this.h_vbr_toc = new byte[100];
            int lengthx = 10;
            System.arraycopy(firstframe, var9 + lengthx, tmp, 0, tmp.length);
            this.h_vbr_bytes = tmp[0] << 24 & 0xFF000000 | tmp[1] << 16 & 0xFF0000 | tmp[2] << 8 & 0xFF00 | tmp[3] & 255;
            lengthx += 4;
            System.arraycopy(firstframe, var9 + lengthx, tmp, 0, tmp.length);
            this.h_vbr_frames = tmp[0] << 24 & 0xFF000000 | tmp[1] << 16 & 0xFF0000 | tmp[2] << 8 & 0xFF00 | tmp[3] & 255;
