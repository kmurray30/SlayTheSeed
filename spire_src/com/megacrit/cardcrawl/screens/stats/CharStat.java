...
public class CharStat {
   ...
   public CharStat(ArrayList<CharStat> allChars) {
      ...
      for (CharStat stat : allChars) {
         this.cardsUnlocked = this.cardsUnlocked + stat.cardsUnlocked;
         this.relicsUnlocked = this.relicsUnlocked + stat.relicsUnlocked;
         ...
         this.totalFloorsClimbed = this.totalFloorsClimbed + stat.totalFloorsClimbed;
         this.numVictory = this.numVictory + stat.numVictory;
         this.numDeath = this.numDeath + stat.numDeath;
         this.enemyKilled = this.enemyKilled + stat.enemyKilled;
         this.bossKilled = this.bossKilled + stat.bossKilled;
         this.playTime = this.playTime + stat.playTime;
      }
      ...
      int unlockedCardCount = UnlockTracker.unlockedRedCardCount
         + UnlockTracker.unlockedGreenCardCount
         + UnlockTracker.unlockedBlueCardCount
         + UnlockTracker.unlockedPurpleCardCount;
      int lockedCardCount = UnlockTracker.lockedRedCardCount
         + UnlockTracker.lockedGreenCardCount
         + UnlockTracker.lockedBlueCardCount
         + UnlockTracker.lockedPurpleCardCount;
      ...
      if (this.fastestTime != 999999999999L) {
         this.info2 = this.info2 + TEXT[13] + formatHMSM(this.fastestTime) + " NL ";
      }
   }
   ...
   public CharStat(AbstractPlayer c) {
      ...
      if (this.fastestTime != 0L) {
         this.info = this.info + TEXT[13] + formatHMSM(this.fastestTime) + " NL ";
      }
      ...
      for (FileHandle file : files) {
         try {
            ...
            file.delete();
            logger.warn("Deleted corrupt .run file, preventing crash!", (Throwable)var9);
         }
      }
   }
   ...
   public static String formatHMSM(int t) {
      ...
      int minutes = (int)(duration % 60L);
      int hours = t / 3600;
      return String.format(TEXT[28], hours, minutes, seconds);
   }
