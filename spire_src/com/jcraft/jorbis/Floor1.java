...
class Floor1 extends FuncFloor {
   ...
   void pack(Object i, Buffer opb) {
      ...
      for (int jx = 0; jx < maxclass + 1; jx++) {
         opb.write(info.class_dim[jx] - 1, 3);
         opb.write(info.class_subs[jx], 2);
         if (info.class_subs[jx] != 0) {
            opb.write(info.class_book[jx], 8);
         }

         for (int k = 0; k < 1 << info.class_subs[jx]; k++) {
            opb.write(info.class_subbook[jx][k] + 1, 8);
         }
      }
      ...
      for (int k = 0; jx < info.partitions; jx++) {
         for (count += info.class_dim[info.partitionclass[jx]]; k < count; k++) {
   ...
   Object unpack(Info vi, Buffer opb) {
      ...
      for (int jx = 0; jx < maxclass + 1; jx++) {
         info.class_dim[jx] = opb.read(3) + 1;
         info.class_subs[jx] = opb.read(2);
         if (info.class_subs[jx] < 0) {
            info.free();
            return null;
         }
         ...
         if (info.class_subs[jx] != 0) {
            info.class_book[jx] = opb.read(8);
         }

         if (info.class_book[jx] < 0 || info.class_book[jx] >= vi.books) {
            info.free();
            return null;
         }

         for (int k = 0; k < 1 << info.class_subs[jx]; k++) {
            info.class_subbook[jx][k] = opb.read(8) - 1;
            if (info.class_subbook[jx][k] < -1 || info.class_subbook[jx][k] >= vi.books) {
               info.free();
               return null;
            }
         }
      ...
      for (int kx = 0; jx < info.partitions; jx++) {
         for (count += info.class_dim[info.partitionclass[jx]]; kx < count; kx++) {
            int t = info.postlist[kx + 2] = opb.read(rangebits);
   ...
   Object look(DspState vd, InfoMode mi, Object i) {
      ...
      for (int jx = 0; jx < _n - 1; jx++) {
         for (int k = jx; k < _n; k++) {
            if (info.postlist[sortpointer[jx]] > info.postlist[sortpointer[k]]) {
               j = sortpointer[k];
               sortpointer[k] = sortpointer[jx];
               sortpointer[jx] = j;
            }
         }
      }

      for (int jx = 0; jx < _n; jx++) {
         look.forward_index[jx] = sortpointer[jx];
      }

      int jx = 0;

      while (jx < _n) {
         look.reverse_index[look.forward_index[jx]] = jx++;
      }

      for (int jxx = 0; jxx < _n; jxx++) {
         look.sorted_index[jxx] = info.postlist[look.forward_index[jxx]];
      }
      ...
      for (int jxx = 0; jxx < _n - 2; jxx++) {
         int lo = 0;
         int hi = 1;
         int lx = 0;
         int hx = look.n;
         int currentx = info.postlist[jxx + 2];

         for (int kx = 0; kx < jxx + 2; kx++) {
            int x = info.postlist[kx];
            if (x > lx && x < currentx) {
               lo = kx;
               lx = x;
            }
         ...
         look.loneighbor[jxx] = lo;
         look.hineighbor[jxx] = hi;
      }
   ...
   Object inverse1(Block vb, Object ii, Object memo) {
      ...
      if (vb.opb.read(1) == 1) {
         ...
         for (int ix = 2; ix < look.posts; ix++) {
            int predicted = render_point(
               info.postlist[look.loneighbor[ix - 2]],
               info.postlist[look.hineighbor[ix - 2]],
               fit_value[look.loneighbor[ix - 2]],
               fit_value[look.hineighbor[ix - 2]],
               info.postlist[ix]
            );
            int hiroom = look.quant_q - predicted;
            int room = (hiroom < predicted ? hiroom : predicted) << 1;
            int val = fit_value[ix];
            if (val != 0) {
               if (val >= room) {
                  if (hiroom > predicted) {
                     val -= predicted;
               ...
               fit_value[ix] = val + predicted;
               fit_value[look.loneighbor[ix - 2]] = fit_value[look.loneighbor[ix - 2]] & 32767;
               fit_value[look.hineighbor[ix - 2]] = fit_value[look.hineighbor[ix - 2]] & 32767;
            } else {
               fit_value[ix] = predicted | 32768;
            }
         }
   ...
   private static void render_line(int x0, int x1, int y0, int y1, float[] d) {
      ...
      for (d[x0] *= FLOOR_fromdB_LOOKUP[y0]; ++x < x1; d[x] *= FLOOR_fromdB_LOOKUP[y]) {
         err += ady;
         if (err >= adx) {
            err -= adx;
            y += sy;
