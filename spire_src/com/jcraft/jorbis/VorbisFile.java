...
public class VorbisFile {
   ...
   private int get_prev_page(Page page) throws JOrbisException {
      ...
      int retx = this.get_next_page(page, 8500L);
      return retx < 0 ? -129 : offst;
   }
   ...
   int bisect_forward_serialno(long begin, long searched, long end, int currentno, int m) {
      ...
      int retx = this.get_next_page(page, -1L);
      if (retx == -128) {
         return -128;
      } else {
         if (searched < end && retx != -1) {
            retx = this.bisect_forward_serialno(next, this.offset, end, page.serialno(), m + 1);
            if (retx == -128) {
               return -128;
            }
         } else {
   ...
   int process_packet(int readp) {
      ...
      while (true) {
         if (this.decode_ready) {
            ...
            if (result > 0) {
               ...
               if (this.vb.synthesis(op) == 0) {
                  ...
                  this.samptrack = this.samptrack + (this.vd.synthesis_pcmout((float[][][])null, null) - oldsamples);
                  this.bittrack = this.bittrack + op.bytes * 8;
         ...
         this.bittrack = this.bittrack + og.header_len * 8;
         ...
         if (!this.decode_ready) {
            if (!this.seekable) {
               ...
               while (i < this.links && this.serialnos[i] != this.current_serialno) {
                  i++;
               }
   ...
   public int bitrate(int i) {
      if (i >= this.links) {
         ...
         } else if (this.vi[i].bitrate_nominal > 0) {
            return this.vi[i].bitrate_nominal;
         } else if (this.vi[i].bitrate_upper > 0) {
   ...
   public int bitrate_instant() {
      ...
      if (this.samptrack == 0.0F) {
         ...
         int ret = (int)(this.bittrack / this.samptrack * this.vi[_link].rate + 0.5);
         this.bittrack = 0.0F;
         this.samptrack = 0.0F;
         return ret;
      }
   ...
   public int raw_seek(int pos) {
      if (!this.seekable) {
         return -1;
      } else if (pos >= 0 && pos <= this.offsets[this.links]) {
         this.pcm_offset = -1L;
         this.decode_clear();
      ...
      if (!this.seekable) {
         ...
         if (this.raw_seek(best) != 0) {
            ...
            while (this.pcm_offset < pos) {
               int targetx = (int)(pos - this.pcm_offset);
               ...
               if (samples < targetx && this.process_packet(1) == 0) {
                  this.pcm_offset = this.pcm_total(-1);
               }
            }
   ...
   public Info getInfo(int link) {
      if (this.seekable) {
         if (link < 0) {
            ...
            return link >= this.links ? null : this.vi[link];
         }
      } else {
   ...
   public Comment getComment(int link) {
      if (this.seekable) {
         if (link < 0) {
            ...
            return link >= this.links ? null : this.vc[link];
         }
      } else {
                  ...
                  if (host_endian == bigendianp) {
                     if (sgned != 0) {
                        for (int i = 0; i < channels; i++) {
                           ...
                           for (int j = 0; j < samples; j++) {
                              int val = (int)(pcm[i][src + j] * 32768.0 + 0.5);
                              if (val > 32767) {
                                 val = 32767;
                              } else if (val < -32768) {
                                 val = -32768;
