...
public class LwjglAWTInput implements Input, MouseMotionListener, MouseListener, MouseWheelListener, KeyListener {
   Pool<LwjglAWTInput.KeyEvent> usedKeyEvents = new Pool<LwjglAWTInput.KeyEvent>(16, 1000) {
      protected LwjglAWTInput.KeyEvent newObject() {
         return LwjglAWTInput.this.new KeyEvent();
      }
   };
   Pool<LwjglAWTInput.TouchEvent> usedTouchEvents = new Pool<LwjglAWTInput.TouchEvent>(16, 1000) {
      protected LwjglAWTInput.TouchEvent newObject() {
         return LwjglAWTInput.this.new TouchEvent();
      }
   };
   private final LwjglAWTCanvas lwjglAwtCanvas;
   List<LwjglAWTInput.KeyEvent> keyEvents = new ArrayList<>();
   List<LwjglAWTInput.TouchEvent> touchEvents = new ArrayList<>();
   int touchX = 0;
   int touchY = 0;
   int deltaX = 0;
   int deltaY = 0;
   ...
   public int getX(int pointer) {
      return pointer == 0 ? this.touchX : 0;
   }
   ...
   public int getY(int pointer) {
      return pointer == 0 ? this.touchY : 0;
   }
   ...
   public boolean isTouched(int pointer) {
      return pointer == 0 ? this.touchDown : false;
   }
   ...
   void processEvents() {
      synchronized (this) {
         ...
         if (this.processor != null) {
            ...
            for (int i = 0; i < len; i++) {
               LwjglAWTInput.KeyEvent e = this.keyEvents.get(i);
            ...
            for (int i = 0; i < len; i++) {
               LwjglAWTInput.TouchEvent e = this.touchEvents.get(i);
               this.currentEventTimeStamp = e.timeStamp;
               switch (e.type) {
                  case 0:
            ...
            for (int i = 0; i < len; i++) {
               LwjglAWTInput.TouchEvent event = this.touchEvents.get(i);
               if (event.type == 0) {
                  this.justTouched = true;
               }
   ...
   public void mouseDragged(MouseEvent e) {
      synchronized (this) {
         LwjglAWTInput.TouchEvent event = this.usedTouchEvents.obtain();
         event.pointer = 0;
         event.x = e.getX();
         event.y = e.getY();
         event.type = 2;
   ...
   private int toGdxButton(int swingButton) {
      if (swingButton == 1) {
         return 0;
      } else if (swingButton == 2) {
         return 2;
      } else {
         return swingButton == 3 ? 1 : 0;
      }
   }
   ...
   public void keyPressed(java.awt.event.KeyEvent e) {
      synchronized (this) {
         LwjglAWTInput.KeyEvent event = this.usedKeyEvents.obtain();
         event.keyChar = 0;
   ...
   private void showCursor(boolean visible) {
      if (!visible) {
         ...
         Image i = new BufferedImage(1, 1, 2);
         Cursor noCursor = t.createCustomCursor(i, new Point(0, 0), "none");
         JFrame frame = findJFrame(this.canvas);
